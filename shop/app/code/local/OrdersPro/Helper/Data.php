...
class MageWorx_OrdersPro_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function isKeepPurchaseShipping() {
        return Mage::getStoreConfig('mageworx_sales/orderspro/keep_purchase_shipping');
    }
    ...
    public function deleteOrderCompletelyById($order) {        
        ...
        if ($orderId) {
            // cancel
            try {$order->cancel()->save();} catch (Exception $e) {}
            // delete            
            if ($order->getQuoteId()) {
