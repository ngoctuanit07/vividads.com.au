...
{   
    ...
    public function toOptionArray($isMultiselect=false) {
        ...
        $options = array(            
            ...
            array('value'=>'base_internal_credit', 'label'=> $helper->__('Internal Credit (Base)')), // 19
            array('value'=>'internal_credit', 'label'=> $helper->__('Internal Credit (Purchased)')), // 20
        ...
        if (!Mage::getConfig()->getModuleConfig('MageWorx_CustomerCredit')->is('active', true)) {
            unset($options[19]); // Internal Credit (Base)
            unset($options[20]); // Internal Credit (Purchased)
        }                
