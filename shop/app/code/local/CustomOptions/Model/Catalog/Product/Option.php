        ...
        if (Mage::app()->getStore()->isAdmin() && Mage::app()->getRequest()->getControllerName()=='catalog_product') {
            ...
            if ($helper->isCustomerGroupsEnabled()) {
                ...
                if (Mage::app()->getStore()->isAdmin()) {                    
                    ...
                    if ($sessionQuote) $customerGroupId = $sessionQuote->getCustomer()->getGroupId();                    
                } else {                
                ...
                foreach ($collection as $option) {
                    ...
                    if ($this->getGroupByType($optionType)==self::OPTION_GROUP_SELECT) {
                        if (count($option->getValues())>0) {
                            foreach ($option->getValues() as $value) {
                                ...
                                if ($isSkuPrice) {
                                    ...
                                    if ($product->getTaxClassId()!=$taxClassId) $value->setTaxClassId($taxClassId);
                        ...
                        if ($isSkuPrice) {
                            ...
                            if ($product->getTaxClassId()!=$taxClassId) $option->setTaxClassId($taxClassId);
