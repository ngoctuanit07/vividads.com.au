  ...
  toString:function(){
   return "stack size = " + stack.length;
	...
	window.onload	= function()
	{
		...
		var elem		= document.getElementById("sales_order_view_tabs_order_info_content");
		...
		content			+= ".headings2 { background: transparent url(../../../../../../skin/adminhtml/default/default/images/sort_row_bg3.gif) repeat-x scroll 0 50% !important; }";
		content			+= ".entry-edit-head2 { background-color:transparent !important; }";
		content			+= ".entry-edit-head2 h4 { color: #333333 !important; }";
		content			+= ".vendors-order-tables tbody.even tr { background-color: #FEFFCC !important; }";
		content			+= "</style>";
		content		 	+= "<div class=\"entry-edit\" style=\"display:none\">";
		content			+= "            <div class=\"entry-edit-head entry-edit-head2\">";
		...
		content			+= "<div class=\"grid np\" style=\"display:none\" >";
		content			+= "	<div class=\"hor-scroll\">";
		content			+= "		<table cellspacing=\"0\" class=\"data order-tables vendors-order-tables\">";
		content			+= "			<col/>";
		...
		content			+= "			<col/>";
		content			+= "			<thead>";
		content			+= "				<tr class=\"headings headings2\">";
		content			+= "					<th class=\"a-center\">Vendor</th>";
		content			+= "					<th><span class=\"nobr\">Products</span></th>";
		content			+= "					<th class=\"a-center\"><span class=\"nobr\">Qty</span></th>";
		content			+= "					<th class=\"a-right\"><span class=\"nobr\">Item Price</span></th>";
		content			+= "					<th class=\"a-right\"><span class=\"nobr\">Commission</span></th>";
		content			+= "					<th class=\"a-right\"><span class=\"nobr\">Item Shipping Cost</span></th>";
		content			+= "					<th class=\"a-right last\"><span class=\"nobr\">Total</span></th>";
		content			+= "				</tr>";
		content			+= "			</thead>";
			...
			if(i in orderData && !isNaN(i))
			{
				...
				for(var j in orderData[i]["items"])
				{
					if(j in orderData[i]["items"] && !isNaN(j))
					{
						...
						content			+= "				<tr class=\"border\">";
						content			+= "					<td >"+orderData[i]["items"][j]["name"]+"</td>";
						content			+= "					<td class=\"a-center\">"+qty_ordered+"</td>";
						content			+= "					<td class=\"a-right\">$"+sprintf("%01.2f", price)+"</td>";
						content			+= "					<td class=\"a-right\">$"+sprintf("%01.2f", commission)+"</td>";
						content			+= "					<td class=\"a-right\">$"+sprintf("%01.2f", shipping_cost)+"</td>";
				...
				// place the "total" row
				classname			= classname=="" || classname=="odd" ? "even" : "odd";
				var vendorIsPayed	= paymentData[orderData[i]["vendor_data"]["vendor_id"]];
				
				content			+= "			<tbody class=\""+classname+"\">";
				content			+= "				<tr class=\"border\">";
				content			+= "					<td class=\"a-center\" valign=\"middle\" colspan=\"4\">";
				content			+= "					</td>";
				content			+= "					<td class=\"a-right\" valign=\"middle\">";
		...
		for(var ielem in elem.childNodes)
		{
			if(!parentdiv && elem.childNodes[ielem].nodeName=="DIV")
			{
				parentdiv 	= elem.childNodes[ielem];
			}
		}
		...
		if(parentdiv!==false)
		{
			for(var iparentdiv in parentdiv.childNodes)
			{
				...
				if(parentdiv.childNodes[iparentdiv].className=="entry-edit" && count<=countlimit)
				{
					count++;
					if(count==countlimit)
					{
						parentdiv.childNodes[iparentdiv].innerHTML	= content + parentdiv.childNodes[iparentdiv].innerHTML;
					}
				}
			}
		}
