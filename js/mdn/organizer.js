...
function SubmitForm(taskId, gridGuid)
{
    ...
    var request = new Ajax.Request(
		...
		if(document.getElementById('ot_event'))
		taskId = '';
			...
			if(document.getElementById('edit_form_task_' + taskId))
			document.getElementById('edit_form_task_' + taskId).reset();
			
			var event = document.getElementById('ot_event').value.split('@');
			
			if(document.getElementById('ot_event').value != '')
			{ 
			    var date = document.getElementById('nowdate').value.split('-');
			    var catformat = document.getElementById('catformat').value;
			    sc_loadXMLDoc(date[1],date[0],'',date[2],catformat);
			    closetask();
			}
			else if(document.getElementById('ot_todo')){
			    closetask1();
			    var date = document.getElementById('nowdate').value.split('-');
			    var catformat = document.getElementById('catformat').value;
			    sc_loadXMLDoc(date[1],date[0],'',date[2],catformat);
...
function SubmitForm1(taskId, gridGuid)
{
    //Soumet la requete en ajax
    var url;
    url = OrganizerSaveTaskUrl;
    ...
    var request = new Ajax.Request(
        url,
        {
			...
			//sc_loadXMLDoc(event[0],event[1],event[2]);
			 var date = document.getElementById('nowdate').value.split('-');
			var catformat = document.getElementById('catformat').value;
