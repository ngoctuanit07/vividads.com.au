...
$resource = Mage::getSingleton('core/resource');
	...
	//echo "hello";
        ...
        $sql="DELETE FROM `".$tableName."` WHERE calculator_id=".$rowid;
           
	$result=$Readconn->query("DELETE FROM `".$tableName."` WHERE calculator_id=".$rowid);
        }
	if($result)
	{
        $SelectSubscriptioId= "SELECT * FROM `".$tableName."` WHERE session_id='".$sessionId."'";
	$rows = $Readconn->fetchAll($SelectSubscriptioId);//print_r($rows);?>
        ...
        {
			//echo $row['calculator_id'];?>
        			
				<li class="quoteli">
					<div class="bin_image">
						<img class="bin-icon" alt="<?php echo $row['calculator_id'];?>" src="http://192.168.0.55/tablethrows/skin/frontend/default/durabanners/images/calculator/removex.png">
					</div>
					<div class="qty_cal">
					...
					<div class="itam_cal">
						<span class="txt_item"><b><?php echo $row['Width'].'mm X'.$row['Height'].'mm'.$Pname.'<span class="txt_item_custom">'.$row['Finish'].'</span>'.'<span class="txt_item_sweing">Sweing:'.$row['Sewing'].'</span>';
                                                ?></b></span>
                                                <span class="links">
