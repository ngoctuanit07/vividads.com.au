...
<div class="label_1" id="custom_id_sw">
    <?php 
									...
									$OptioTitle='';$Procollection = Mage::getModel('catalog/product')->load($proId);
                                                                        $imgoption='';
									foreach($Procollection->getOptions() as $optionKey => $optionVal) {
									...
									<?php
                                                                        if($OptioTitle !='Top' && $OptioTitle !='Bottom' && $OptioTitle !='Left' && $OptioTitle !='Right' && $OptioTitle !='Spacing')
									{
...
?>
									<?php if($valuesVal->getSku() == ''){?>
									<li class="sub_label_2 <?php if($valuesVal->getId()==177) { echo "customtitle_top1"; } else if($valuesVal->getId()==178) { echo "customtitle_top2"; }  else if($valuesVal->getId()==179)  { echo "customtitle_top_tr"; } else {echo "nocustomtitle";} ?>"><span class="whole_customoption_txt"><span class="customoption_txt_<?php echo $OptioTitle?>_val" id="<?php echo $valuesVal->getId()?>"><?php echo $valuesVal->getTitle()?></span><input type="hidden" id="cusoptioid" value="<?php echo $valuesVal->getId()?>" /></span></span></li>
									<?php } else{?>
									<li class="sub_label_2 <?php if($valuesVal->getId()==177) { echo "customtitle_top1"; } else if($valuesVal->getId()==178) { echo "customtitle_top2"; }  else if($valuesVal->getId()==179)  { echo "customtitle_top_tr"; } else {echo "nocustomtitle";} ?>"><span class="total_img_customoption"><span class="customoption_img"><img alt="customoption" src="<?php echo Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA)?>imageoptions/<?php echo $Procollection->getId() ?>/<?php echo $fetchSelectOI['option_id']."_".$valuesVal->getId()?>/<?php echo $fetchSelectOI['image']?>"></span> <span class="whole_customoption_txt1"><span class="customoption_txt_<?php echo str_replace(' ', '', $OptioTitle);?>_val"><?php echo $valuesVal->getTitle()?></span><span class="Text_customoption"><?php echo $valuesVal->getSku()?></span><input type="hidden" id="cusoptioid" value="<?php echo $valuesVal->getId()?>" /></span></span></li><?php }?>
									<?php }?>
								</ul>
								</div>
</div>
