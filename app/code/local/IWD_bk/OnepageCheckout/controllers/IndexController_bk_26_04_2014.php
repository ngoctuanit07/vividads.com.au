...
///////////////
		...
		/*
		$intData = $this->getRequest()->getPost('installment', array());
        $insResult = $this->getOnepagecheckout()->saveInstallment($intData);
		
		if (empty($intData['error'])) {
			if ($this->getOnepagecheckout()->getQuote()->getUseInstallments() == Itweb_Installments_Helper_Data::PAY_INSTALLMENT) {
				$block = $this->_getUpdatedLayout()->createBlock(
                ...
                $this->_getUpdatedLayout()->getBlock('checkout.onepage.review.info.items.after')->append($block);
			}
		}
	...
	/*function saving order */
	
	
    
	/*------------------------------------------------ */
	/*public Save Order Action  for saving order */
...
//				$result = $this->getOnepagecheckout()->saveShipping($this->_filterPostData($this->getRequest()->getPost('shipping', array())),$this->getRequest()->getPost('shipping_address_id', false));
	...
	if($agreements)
            ...
            $_order_comments = Mage::getSingleton('customer/session')
							   		->setOrderCustomerComment(
							   							$this->getRequest()
            ...
            if (!isset($result['redirect']) && !isset($result['error']))
				...
				if ($pmnt_data)
                    $this->getOnepagecheckout()
					     ->getQuote()
						 ->getPayment()
						 ->importData($pmnt_data);
				
				/*saving data and order*/
             
			     
				 $order_saved = $this->getOnepagecheckout()->saveOrder();
		...
		$connectionWrite = Mage::getSingleton('core/resource')->getConnection('core_write');
            ...
            if(Mage::getSingleton('core/session')->getQuotedetails() != '')
            {
                    ...
                    $data = array();
                    $data2['increment_id'] = $quotedata[0];
                    ...
                    $connectionWrite->beginTransaction();
                    $data1 = array();
                    $data1['increment_id'] = $quotedata[0];
