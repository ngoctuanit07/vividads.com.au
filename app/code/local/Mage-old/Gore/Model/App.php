...
class Mage_Core_Model_App
{
    ...
    */
    ...
    {
	     if(stripos(Mage::app()->getRequest()->getRequestUri(),'vendorlist/?') === false and stripos(Mage::app()->getRequest()->getRequestUri(),'vendorlist?') === false and stripos(Mage::app()->getRequest()->getRequestUri(),'vendorlist')){
	       	 $vms = Mage::app()->getRequest()->getRequestUri();
	       	 $is_name_append = str_replace('/','',substr($vms,stripos($vms,'vendorlist')+10,strlen($vms)));
	       	 if(isset($is_name_append)&&strlen($is_name_append) > 0) {
				 $fp = substr($vms,0,strrpos($vms,'/')+1);
				 $lp = substr($vms,strrpos($vms,'/')+1,strlen($vms));
				 $nvms = $fp.'all/index/name/'.$lp;
				 Mage::app()->getRequest()->setRequestUri($nvms);
	       	 }
	      }
