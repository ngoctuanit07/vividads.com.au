...
class Artis_Eventcalendar_Adminhtml_EventcalendarController extends Mage_Adminhtml_Controller_Action
{
	...
	//										  <table class="eventtab" style="width:100%;height:100%;">';
		...
		if(in_array(5,$all_permission))
		{
			$calendar .= '<td class="tabnew" style="width:2%" onclick="addtask('.$month.','.$year.','.$day.',\''.$ftime.'\')"></td>';
		}
		...
		if(in_array(5,$all_permission))
		{
			$calendar .= '<td class="tabnew" style="width:2%"  onclick="addtask('.$month.','.$year.','.$day.',\''.$stime.'\')"></td>';
		}
	...
	function daydetailsAction()
	{
		...
		if(Mage::getSingleton('core/resource')->getConnection('core_write')->isTableExists($temptableProof))
			...
			if($chkProofs)
			{
				...
				foreach($chkProofs as $chkProof)
				{
					...
					if($chkProof['proof_type'] == 'order')
					{
						$url= $this->getUrl('admin/sales_order/view/order_id/'.$chkProof['order_id']);
					...
					echo  '<div '.$css.' style="text-align: left;" > <a href="'.$url.'" target="_blank">'.$proof_caption.'...Proof Approval Waiting</a></div>';
					$i++;
				}
			}
		}
		...
		if(Mage::getSingleton('core/resource')->getConnection('core_write')->isTableExists($temptableDesign))
			...
			if($chkDesigns)
			{
				...
				foreach($chkDesigns as $chkDesign)
				{
					...
					if($chkDesign['proof_type'] == 'order')
					{
						$url= $this->getUrl('admin/sales_order/view/order_id/'.$chkDesign['order_quote_id']);
						$url = str_replace('p//s','p/admin/s',$url);
					}
					else if($chkDesign['proof_type'] == 'quote')
					{
					...
					if(in_array($chkItem[0]['product_id'],$pro) or (trim(Mage::getSingleton('core/session')->getPro()) == ''))
					echo  '<div '.$css.' style="text-align: left;" > <a href="'.$url.'" target="_blank">'.$proof_caption.'...'.$chkDesign['status'].'</a></div>';
					$i++;
				}
			}
		}
		...
		if(Mage::getSingleton('core/resource')->getConnection('core_write')->isTableExists($temptableQuote))
			...
			if($chkQuotes)
			{
				echo '<div class="deatilshead">Quotes :</div>';
				$i=1;
		...
		if($chkInvoices)
		{
			echo '<div class="deatilshead">Invoices :</div>';
			$i=1;
			foreach($chkInvoices as $chkInvoice)
			{
				...
				$url= $this->getUrl('admin/sales_invoice/view/invoice_id/'.$chkInvoice['entity_id']);
				$url = str_replace('p//s','p/admin/s',$url);
		...
		if($chkShipments)
		{
			echo '<div class="deatilshead">Delivaries :</div>';
			$i=1;
			foreach($chkShipments as $chkShipment)
			{
				...
				$url= $this->getUrl('admin/sales_shipment/view/shipment_id/'.$chkShipment['entity_id']);
				$url = str_replace('p//s','p/admin/s',$url);
			
			
				echo  '<div '.$css.' style="text-align: left;" > <a href="'.$url.'" target="_blank">'.$chkShipment['increment_id'].'</a></div>';
				$i++;
			}
		}
