			...
			foreach($chkItems as $chkItem)
			{
				...
				if($connectionWrite->isTableExists($temptableSaleDate))
				{
					...
					$sqlDate = $connectionRead->select()
							...
							->where('product_id=?', $chkItem, 'shipping_date=?', $date);
			...
			if($task['ot_entity_type'] != 'product')
			{
				...
				if($connectionWrite->isTableExists($temptableChain))
				{
					...
					$sqlChain = $connectionRead->select()
							...
							->where('order_quote_id=?',$task['ot_entity_id'],'task_status=?','','product_id!=?','','task_type=?','Chain')
							->order('product_id');
...
//	if($enabled)
	       ...
	       foreach($chkOrganiger as $task)
		{
			if($task['ot_entity_type'] != 'product')
			{
				...
				if($connectionWrite->isTableExists($temptableChain))
				{
					...
					$sqlChain = $connectionRead->select()
							->from($temptableChain, array('*'))
							->where('order_quote_id=?',$task['ot_entity_id'],'task_status=?','','product_id!=?','')
							->group('product_id');
					$chkChain1 = $connectionRead->fetchAll($sqlChain);
				}
				foreach($chkChain1 as $Chain)
		...
		if(in_array(5,$all_permission))
		{
			...
			$calendar .='<div class="tot_ord_det">';
					   ...
					   foreach($show_id as $vvl)
					   {
					    ...
					    $sqlSaleOrder = $connectionRead->select()
								->from($temptableSaleOrder, array('created_at'))
								->where('entity_name','shipment','parent_id', $ord_id[$ctctct]);
	...
	public function loadtodoAction()
	{
		...
		$sqlTodo = $connectionRead->select()
			...
			->where('user_id=?',$userId,'list_id=?',$list_id);
	...
	public function todolistdeleteAction()
	{
		...
		$sqlTodo = $connectionRead->select()
			->from($temptableTodo, array('*'))
			->where('list_id=?',$chkTodo1[0]['list_id'],'user_id=?',$userId);
		...
		$lastInsertId = $connectionWrite->lastInsertId();
		...
		$sqlTodo = $connectionRead->select()
			->from($temptableTodo, array('*'))
			->where('list_id=?',$lastInsertId, 'user_id=?',$userId);
	...
	public function edittodotaskAction()
	{
                ...
                if($connectionWrite->isTableExists($temptableChain))
		{
                ...
                $sqlChain = $connectionRead->select()
			->from($temptableChain, array('*'))
			->where('task_id=?',$chkOrganiger[0]['ot_id'],'order_quote_id=?',$chkOrganiger[0]['ot_entity_id']);
