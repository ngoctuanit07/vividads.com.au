...
class Artis_Eventcalendar_Adminhtml_EventcalendarController extends Mage_Adminhtml_Controller_Action
{
	...
	if($connectionWrite->isTableExists($temptableHoliday))
		...
		$sqlHoliday = $connectionRead->select()
                    ...
                    ->where('entity_id=?', $holiday_country);
	...
	public function getUnshipproduct()
	{
		...
		//	->from($temptableSaleitem, array($temptableSaleitem.'.product_id'))
		//	->joinLeft(array('relation' => $temptableShipitem), $temptableSaleitem.'.item_id != relation.order_item_id')
		//	->group($temptableSaleitem.'.product_id');
	...
	public function chkshipcolor($date)
	{
		...
		if(trim(Mage::getSingleton('core/session')->getPro(),' ') != '')
		{
			...
			foreach($chkItems as $chkItem)
			{
				...
				if($connectionWrite->isTableExists($temptableSaleDate))
				{
					...
					$sqlDate = $connectionRead->select()
							->from($temptableSaleDate, array('*'))
							->where("product_id = '".$chkItem."' AND shipping_date = '".$date."'");
							
					$chkDate = $connectionRead->fetchAll($sqlDate);
	...
	foreach($alldate as $alldateview)
	{
		...
		if($connectionWrite->isTableExists($temptableProof))
		{
			...
			$sqlProof = $connectionRead->select()
				->from($temptableProof, array('*'))
				->where("proof_date LIKE  '%".$current_year."-".$current_month."-".$current_date1."%'");
			$chkProofs = $connectionRead->fetchAll($sqlProof);
		}
		if($chkProofs)
		{
	...
	foreach($alldate as $alldateview)
	{
		...
		if(in_array(1,$all_permission))
		{
			    ...
			    if($connectionWrite->isTableExists($temptableOrganiger))
				{
					...
					//	->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_created_at = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_notify_date = '".$current_year.'-'.$current_month.'-'.$current_date."')".$sql_pro);
					//
					...
					//$sqlOrganiger = $connectionRead->select()
					//	->from($temptableOrganiger, array('*'))
					//	->where("ot_deadline = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_created_at = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_notify_date = '".$current_year.'-'.$current_month.'-'.$current_date."')".$sql_pro);
					//
					else
					...
					//$sqlOrganiger = $connectionRead->select()
					//	->from($temptableOrganiger, array('*'))
					//	->where("ot_target_user = '".$userId."' AND ot_deadline = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_created_at = '".$current_year.'-'.$current_month.'-'.$current_date."' OR ot_notify_date = '".$current_year.'-'.$current_month.'-'.$current_date."') ".$sql_pro);
					//   
					//$chkOrganiger[$alldateview] = $connectionRead->fetchAll($sqlOrganiger);
				}
		}
			...
			if($task['ot_entity_type'] != 'product')
			{
				...
				if($connectionWrite->isTableExists($temptableChain))
				{
					...
					$sqlChain = $connectionRead->select()
							...
							->where("order_quote_id = '".$task['ot_entity_id']."' AND task_status = '' AND product_id != '' AND task_type = 'Chain'")
							->order('product_id');
					$chkChain1 = $connectionRead->fetchAll($sqlChain);
				}
				$current_pro = '';
	...
	$sqlProof = $connectionRead->select()
		->from($temptableProof, array('*'))
		->where("proof_date LIKE  '%".$year."-".$month1."-".$day2."%'");
	$chkProofs = $connectionRead->fetchAll($sqlProof);
	}
	if($chkProofs)
	{
	    ...
	    if($connectionWrite->isTableExists($temptableOrganiger))
		{
			...
			if(in_array(1,$all_permission))
			{
			    ...
			    if($roleName == 'Administrators' and $user != '')
			     ...
			     $sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$year.'-'.$month.'-'.$day."' OR ot_created_at = '".$year.'-'.$month.'-'.$day."' OR ot_notify_date = '".$year.'-'.$month.'-'.$day."')".$sql_pro);
			     ...
			     //$sqlOrganiger="SELECT * FROM ".$temptableOrganiger." WHERE (ot_deadline = '".$year.'-'.$month.'-'.$day."' OR ot_created_at = '".$year.'-'.$month.'-'.$day."' OR ot_notify_date = '".$year.'-'.$month.'-'.$day."')".$sql_pro;
			      $sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_deadline = '".$year.'-'.$month.'-'.$day."' OR ot_created_at = '".$year.'-'.$month.'-'.$day."' OR ot_notify_date = '".$year.'-'.$month.'-'.$day."'".$sql_pro);
			    ...
			    // $sqlOrganiger="SELECT * FROM ".$temptableOrganiger." WHERE (ot_target_user = '".$userId."' AND ot_deadline = '".$year.'-'.$month.'-'.$day."' OR ot_created_at = '".$year.'-'.$month.'-'.$day."' OR ot_notify_date = '".$year.'-'.$month.'-'.$day."') ".$sql_pro;
			     $sqlOrganiger = $connectionRead->select()
						->from($temptableOrganiger, array('*'))
						->where("ot_target_user = '".$userId."' AND ot_deadline = '".$year.'-'.$month.'-'.$day."' OR ot_created_at = '".$year.'-'.$month.'-'.$day."' OR ot_notify_date = '".$year.'-'.$month.'-'.$day."' ".$sql_pro);
...
//	if($enabled)
	       ...
	       foreach($chkOrganiger as $task)
		{
			if($task['ot_entity_type'] != 'product')
			{
				...
				if($connectionWrite->isTableExists($temptableChain))
				{
					...
					$sqlChain = $connectionRead->select()
							->from($temptableChain, array('*'))
							->where("order_quote_id = '".$task['ot_entity_id']."' AND task_status = '' AND product_id != ''")
							->group('product_id');
					$chkChain1 = $connectionRead->fetchAll($sqlChain);
				}
				foreach($chkChain1 as $Chain)
				...
				while ($currentDay <= $numberDays) {
					...
					if(in_array(1,$all_permission))
					{
						...
						if($connectionWrite->isTableExists($temptableOrganiger))
						{
							...
							$sqlOrganiger = $connectionRead->select()
									->from($temptableOrganiger, array('*'))
									->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_created_at = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_notify_date = '".$year.'-'.$i_m.'-'.$cdate."')".$sql_pro);
							...
							$sqlOrganiger = $connectionRead->select()
									->from($temptableOrganiger, array('*'))
									->where("ot_deadline = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_created_at = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_notify_date = '".$year.'-'.$i_m.'-'.$cdate."'".$sql_pro);
							...
							$sqlOrganiger = $connectionRead->select()
									->from($temptableOrganiger, array('*'))
									->where("ot_target_user = '".$userId."' AND ot_deadline = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_created_at = '".$year.'-'.$i_m.'-'.$cdate."' OR ot_notify_date = '".$year.'-'.$i_m.'-'.$cdate."' ".$sql_pro);
							//$chkOrganiger = $connectionRead->fetchAll($sqlOrganiger);
						}
					}
					...
					if($connectionWrite->isTableExists($temptableProof))
					{
						...
						$sqlProof = $connectionRead->select()
							->from($temptableProof, array('*'))
							->where("proof_date LIKE  '%".$year.'-'.$i_m.'-'.$cdate."%'");
						$chkProofs = $connectionRead->fetchAll($sqlProof);
					}
					if($chkProofs)
					{
	...
	public function eventmonthAction() {
		...
		if(in_array(1,$all_permission))
		{
			...
			if($connectionWrite->isTableExists($temptableOrganiger))
			{
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_created_at = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_notify_date = '".$prev_year.'-'.$prev_month.'-'.$currentday3."')".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_deadline = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_created_at = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_notify_date = '".$prev_year.'-'.$prev_month.'-'.$currentday3."'".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user = '".$userId."' AND ot_deadline = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_created_at = '".$prev_year.'-'.$prev_month.'-'.$currentday3."' OR ot_notify_date = '".$prev_year.'-'.$prev_month.'-'.$currentday3."'".$sql_pro);
				//$chkOrganiger = $connectionRead->fetchAll($sqlOrganiger);
			}
			foreach($chkOrganiger as $task)
			{
		...
		if(in_array(1,$all_permission))
		{
			...
			if($connectionWrite->isTableExists($temptableOrganiger))
			{
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_created_at = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_notify_date = '".$year.'-'.$month.'-'.$currentDay1."')".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_deadline = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_created_at = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_notify_date = '".$year.'-'.$month.'-'.$currentDay1."'".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user = '".$userId."' AND ot_deadline = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_created_at = '".$year.'-'.$month.'-'.$currentDay1."' OR ot_notify_date = '".$year.'-'.$month.'-'.$currentDay1."'".$sql_pro);
		...
		/********************************* End by dev ************************************/
		  ...
		  {
		    $calendar .= '<td class="sc_day '.$clclcl.' '.$c_css.'" rel="'.$date.'"><div style="position:relative;  width:100%; height:100%;"><span id="dt__'.$currentDay.'" onclick="daydetails(\''.$year.'-'.$month.'-'.$currentDay1.'\');" class="ddy activated_day '.$css.'" style="display:inline">'.($this->calculateVendorLoad($year.'-'.$month.'-'.$currentDay1).$currentDay);
			...
			$sqlProof = $connectionRead->select()
				->from($temptableProof, array('*'))
				->where("proof_date LIKE  '%".$year."-".$month."-".$currentDay1."%'");
			$chkProofs = $connectionRead->fetchAll($sqlProof);
			}
			if($chkProofs)
			{
		...
		if(in_array(5,$all_permission))
		{
		  ...
		  {
		    $calendar .= '<td class="sc_day '.$clclcl.'" rel="'.$date.'"><div style="position:relative; width:100%; height:100%;"><span class="ddy '.$css.'" style="display:inline" onclick="daydetails(\''.$year.'-'.$month.'-'.$currentDay1.'\');">'.($this->calculateVendorLoad($year.'-'.$month.'-'.$currentDay1).$currentDay);
		...
		if(in_array(1,$all_permission))
		{
			...
			if($connectionWrite->isTableExists($temptableOrganiger))
			{
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user NOT IN(".$user.") AND (ot_deadline = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_created_at = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_notify_date = '".$next_year.'-'.$next_month.'-'.$currentday3."')".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_deadline = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_created_at = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_notify_date = '".$next_year.'-'.$next_month.'-'.$currentday3."'".$sql_pro);
				...
				$sqlOrganiger = $connectionRead->select()
					->from($temptableOrganiger, array('*'))
					->where("ot_target_user = '".$userId."' AND ot_deadline = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_created_at = '".$next_year.'-'.$next_month.'-'.$currentday3."' OR ot_notify_date = '".$next_year.'-'.$next_month.'-'.$currentday3."'".$sql_pro);
    ...
    private function calculateVendorLoad($date){
        //    $vendorcollection=Mage::getModel('vendorload/vendorload')
        //        ->getCollection();
        //    $vendorcollection->getSelect()->columns(array('vendor_load' => new Zend_Db_Expr("SUM(`load`)")));
        //    $vendorcollection->getSelect()->group('admin_id');
        //    foreach($vendorcollection as $c)
        //        $load+=$c->getVendorLoad();
        //
        //
        //return '<span title="Vendor Load" style="float:right; display:inline;"> 8/'.$load.'</span>';
    }
    public function loadtodoAction()
	{
		...
		$sqlTodo = $connectionRead->select()
			...
			->where("user_id = '".$userId."' AND list_id = '".$list_id."'");
	...
	public function todocompleteAction()
	{
		...
		if($chkTodo1[0]['complete'] == 1){
			...
			$data = array();
			$data['complete'] = 0;
			$where = $connectionWrite->quoteInto('ot_id =?', $id);
			$connectionWrite->update($temptableTodo1, $data, $where);
	...
	public function todorenameAction()
	{
		...
		$data = array();
		$data['name'] = $listname;
		$where = $connectionWrite->quoteInto('user_id =?', $userId,'list_id =?', $list_id);
		$connectionWrite->update($temptableTodo1, $data, $where);
		$connectionWrite->commit();
	...
	public function todolistdeleteAction()
	{
		...
		$connectionWrite->beginTransaction();
		$sqlTodo = array($connectionWrite->quoteInto('user_id=?', $userId), $connectionWrite->quoteInto('list_id=?', $list_id));
		$connectionWrite->delete($temptableTodo, $sqlTodo);
		
		$temptableTodo1=Mage::getSingleton('core/resource')->getTableName('todotasklist');
		//$sqlTodo1="DELETE FROM ".$temptableTodo1." WHERE user_id = '".$userId."' AND list_id = '".$list_id."'";
		//$chkTodo1 = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlTodo1);
		$connectionWrite->beginTransaction();
		$sqlTodo1 = array($connectionWrite->quoteInto('user_id=?', $userId), $connectionWrite->quoteInto('list_id=?', $list_id));
		$connectionWrite->delete($temptableTodo1, $sqlTodo1);
		...
		$data = array();
		$data['active'] = 1;
		$where = $connectionWrite->quoteInto('user_id =?', $userId, 'list_id =?', $chkTodo1[0]['list_id']);
		$connectionWrite->update($temptableTodo2, $data, $where);
		$connectionWrite->commit();
		...
		$sqlTodo = $connectionRead->select()
			->from($temptableTodo, array('*'))
			->where("list_id = '".$chkTodo1[0]['list_id']."' AND user_id = '".$userId."'");
	...
	//	$data['name']= $listname;
	//	$data['user_id']=$userId;
	//	$connectionWrite->insert($temptableTodo1, $data);
	//	$connectionWrite->commit();
	//	
	...
	//	$data = array();
	//	$data['active'] = 1;
	//	$where = $connectionWrite->quoteInto('list_id =?', $lastInsertId);
	//	$connectionWrite->update($temptableTodo1, $data, $where);
	//	$connectionWrite->commit();
	//	}
	//	else{
	...
	//	$data = array();
	//	$data['active'] = 1;
	//	$where = $connectionWrite->quoteInto('list_id =?', $listid);
	//	$connectionWrite->update($temptableTodo1, $data, $where);
	//	$connectionWrite->commit();
	//	
	//	$temptableTodo1=Mage::getSingleton('core/resource')->getTableName('todotasklist');
	...
	//	$sqlTodo = $connectionRead->select()
	//		->from($temptableTodo, array('*'))
	//		->where("list_id = '".$lastInsertId."' AND user_id = '".$userId."' ");
	//		
	//	$chkTodo = $connectionRead->fetchAll($sqlTodo);
	//	
	//	$k=1;
	...
	public function todolistAction()
		...
		if($_REQUEST['status'] == 'all')
		{
			...
			$sqlTodo = $connectionRead->select()
				->from($temptableOrganiger, array('*'))
				->where("ot_target_user = '".$userId."' AND ot_finished = '0' ")
				->order('ot_create_time DESC');
				
			$chkTodo1 = $connectionRead->fetchAll($sqlTodo);
			
			$remain = count($chkTodo1) - $_REQUEST['pager'];
			...
			{
				$Task = Mage::getModel('Organizer/Task')->load($todo['ot_id']);
				$complete_date = date("jS F Y", strtotime($Task->getOtCreateTime()));
		...
		echo '<div onclick="listload(\'\',\'all\');" style="cursor:pointer;" >All</div>';
		$temptableTodo=Mage::getSingleton('core/resource')->getTableName('todotasklist');
		//$sqlTodo="SELECT * FROM ".$temptableTodo." WHERE user_id = '".$userId."'";
		//$chkTodos = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchAll($sqlTodo);
		$sqlTodo = $connectionRead->select()
	...
	public function deletetodoAction()
	{
		...
		foreach($todo_ids as $todo_id)
		{
		    ...
		    $connectionWrite->beginTransaction();
		    $sqlOrganiger = array($connectionWrite->quoteInto('ot_id=?', $todo_id));
		    $connectionWrite->delete($temptableOrganiger, $sqlOrganiger);
		    $connectionWrite->commit();
		    ...
		    $connectionWrite->beginTransaction();
		    $sqlTodo = array($connectionWrite->quoteInto('ot_id=?', $todo_id));
		    $connectionWrite->delete($temptableTodo, $sqlTodo);
		    $connectionWrite->commit();
			}
		}
	...
	public function deletetaskAction()
	{
		...
		$chkOrganiger1 = Mage::getSingleton('core/resource')->getConnection('core_read')->query($sqlOrganiger);
		$connectionWrite->beginTransaction();
		$sqlOrganiger = array($connectionWrite->quoteInto('ot_id=?', $task_id));
		$connectionWrite->delete($temptableOrganiger, $sqlOrganiger);
		$connectionWrite->commit();
		...
		//$chkTodo = Mage::getSingleton('core/resource')->getConnection('core_read')->query($sqlTodo);
		$connectionWrite->beginTransaction();
		$sqlTodo = array($connectionWrite->quoteInto('ot_id=?', $task_id));
		$connectionWrite->delete($temptableTodo, $sqlTodo);
		$connectionWrite->commit();
		}
	...
	public function edittodotaskAction()
	{
                ...
                if($connectionWrite->isTableExists($temptableChain))
		{
                ...
                $sqlChain = $connectionRead->select()
			->from($temptableChain, array('*'))
			->where("task_id = '".$chkOrganiger[0]['ot_id']."' AND order_quote_id = '".$chkOrganiger[0]['ot_entity_id']."'");
	...
	public function clockAction()
	{
		...
		if(strpos(Mage::getSingleton('core/session')->getRemoveclock(),$clockid))
		{
		    ...
		    if($connectionWrite->isTableExists($temptableClock))
			{	 
				...
				$connectionWrite->beginTransaction();
				$sqlClock = array($connectionWrite->quoteInto('user_id=?', $userId), $connectionWrite->quoteInto('clock_id=?', $clockid));
				$connectionWrite->delete($temptableClock, $sqlClock);
			}
		     ...
		     if($connectionWrite->isTableExists($temptableClock))
			{
				...
				$data = array();
				$data['user_id']= $userId;
				$data['clock_id']=$clockid;
				$connectionWrite->insert($temptableClock, $data);
				$connectionWrite->commit();
			}
		}
	...
	public function holisubmitAction()
	{
		...
		if($country != '' and $from_date != '' and $caption != '')
		{
		    $temptableHoli=Mage::getSingleton('core/resource')->getTableName('holiday');
		     if($connectionWrite->isTableExists($temptableHoli))
			{
			...
			$data['h_date']=$from_date;
			$data['event']=$caption;
			$data['color']=$chkselect[0]['color'];
			$data['addby']='manually';
			$connectionWrite->insert($temptableHoli, $data);
			$connectionWrite->commit();
			}
		}
		else{
	...
	public function holideleteAction()
	{
		...
		if($holi_id != '')
		{
			...
			//$chkHoli = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlHoli1);
			$connectionWrite->beginTransaction();
			$sqlHoli1 = array($connectionWrite->quoteInto('entity_id=?', $holi_id));
			$connectionWrite->delete($temptableHoli, $sqlHoli1);
			$connectionWrite->commit();
	...
	protected function fetchdaydetails(){
		...
		if($connectionWrite->isTableExists($temptableProof))
			...
			$sqlProof = $connectionRead->select()
					->from($temptableProof, array('*'))
					->where("status = 'Awaiting Proof Approval' AND postdate LIKE  '%".$daydate."%'");
				...
				foreach($chkProofs as $chkProof)
				{
					...
					if($chkProof['proof_type'] == 'order')
					{
						...
						$sqlItem = $connectionRead->select()
								...
								->where('item_id=?',$chkProof['item_id']);
...
// fetch increment id
					...
					else if($chkProof['proof_type'] == 'quote')
					{
						...
						$sqlItem = $connectionRead->select()
								->from($temptableItem, array('*'))
								->where('quotation_item_id=?',$chkProof['item_id']);
						$chkItem = $connectionRead->fetchAll($sqlItem);
...
// fetch increment id
		...
		if($connectionWrite->isTableExists($temptableQuote))
                {
			...
			$sqlQuote = $connectionRead->select()
				->from($temptableQuote, array('*'))
				->where("created_time LIKE  '%".$daydate."%'");
			$chkQuotes = $connectionRead->fetchAll($sqlQuote);
			if($chkQuotes)
			{
		...
		$sqlInvoice = $connectionRead->select()
				->from($temptableInvoice, array('*'))
				->where("created_at LIKE  '%".$daydate."%'");
		$chkInvoices = $connectionRead->fetchAll($sqlInvoice);
		if($chkInvoices)
		{
		...
		$sqlPayment = $connectionRead->select()
				->from($temptablePayment, array('*'))
				->where("created_at LIKE  '%".$daydate."%'");
		$chkPayments = $connectionRead->fetchAll($sqlPayment);
		if($chkPayments)
		{
		...
		$sqlShipment = $connectionRead->select()
				->from($temptableShipment, array('*'))
				->where("created_at LIKE  '%".$daydate."%'");
		$chkShipments = $connectionRead->fetchAll($sqlShipment);
		if($chkShipments)
		{
		...
		if($connectionWrite->isTableExists($temptablePlanning))
			...
			$sqlPlanning = $connectionRead->select()
					...
					->where("order_placed_date LIKE '%".$daydate."%' OR artwork_date LIKE '%".$daydate."%' OR proof_date LIKE '%".$daydate."%' OR start_date LIKE '%".$daydate."%' OR shipping_date LIKE '%".$daydate."%' OR delivery_date LIKE '%".$daydate."%'");
			
			$chkPlannings = $connectionRead->fetchAll($sqlPlanning);
			if($chkPlannings)
			{
