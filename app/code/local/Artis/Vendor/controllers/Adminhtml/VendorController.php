	...
	public function assigntoAction()
	{
		...
		foreach($ot_author_user as $key=>$user_id)
		{
			...
			if(count($chkItem)>0)
			{
				...
				$connectionWrite->update($temptableProduct, $data, $where);
				$connectionWrite->commit();
				...
				//Start 06_03_2014
				if($chkItem[0]['vendor_id'])
				{
					if($chkItem[0]['vendor_id'] != $user_id)
					{
			...
			//	$connectionWrite->insert($temptableService, $data);
			 	$connectionWrite->commit();
				*/
				/*adding data to vendor_product*/
				
					
				$data2['vendor_id'] = $user_id;
				$data2['priority'] = $priority[$key];
				$data2['item_id'] = $key;
				$data2['order_id'] = $orderid;
				$data2['product_id'] = $product_id;
				$data2['product_sku'] = $sku;
				$data2['postdate'] = NOW();
				//var_dump($data2);
				
				$connectionWrite->beginTransaction();
				$connectionWrite->insert($temptableProduct, $data2);
				$connectionWrite->commit();
...
//		</table>
	...
	//start for update 22_04_2014
	$order = Mage::getModel('sales/order')->load($orderid);
	$store_id = $order->getStoreId();
	$store = Mage::app()->getStore($store_id)->getCode();

	 $obj = new Artis_Designer_Block_Adminhtml_Sales_Order_View_Tab();
	 $_customer_files = $obj->getCustomer_files($orderid, $itemid);
	 $_proof_files = $obj->getProof_files($orderid, $itemid);
	 
	//var_dump($_proof_files);
	
	 if(count($_proof_files)>0){
		
		foreach($_proof_files as $_proof_file){	//print_r($_customer_file);
			if($_proof_file['approved_status']==2){

	//echo '<a href="'.str_replace('//s','/admin/s',$url2).'file/'.$chkProof['file'].'/'.'">Download artwork</a>';
	
	echo '<a target="_blank" href="'.$_proof_file['heigh_res_file'].'">Download artwork</a>';
/*
	echo '<a target="_blank" href="/media/stores/'.$store.'/'.date('Y',strtotime($_proof_file['postdate'])).'/'.strtolower(date('M-y',strtotime($_proof_file['postdate']))).'/'.$order->getIncrementId().'/'.$_Product->getSku().'/print_ready_approved_files/'.$_proof_file['file'].'">Download artwork</a>';
	*/
	
			}
		}
	 }
	 //End for update 22_04_2014
	...
	$url = Mage::helper('adminhtml')->getUrl("adminhtml/catalog_product/edit/id/".$product_id.'/store/'.$store);
	$url = str_replace('p//c','p/admin/c',$url);
	//var_dump($url);
	//exit;
        Mage::log($url); //To check if URL is correct (and it is correct)
