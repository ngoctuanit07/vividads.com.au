...
class Artis_Vendor_Block_Adminhtml_Vendor_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
  ...
  protected function _prepareCollection()
  {
	...
	//  Zend_debug::dump(count($collection));exit;
	...
	//Get the role id of the user
      ...
      $roleName = Mage::getModel('admin/roles')->load($roleId)->getRoleName();
	   ...
	   if($roleName == 'Vendor')
	   {
		 ...
		 $collection->addFieldToFilter('file_visible',1);
		 ...
		 $collection->getSelect()->where('`order_status` like (\'%pending%\') OR `order_status` like (\'%processing%\') OR `order_status` like (\'%awaiting_direct_deposit%\') ');
		
		 
	  
	  //  $collection->addFieldToFilter('order_status',array('in'=>array('partial_payment','purchaseorder_pending_payment','processing','pending')));
		// $collection->addFieldToFilter('order_status',array('in'=>array('partial_payment','purchaseorder_pending_payment','pending')));
		
		//$collection->addFieldToFilter('order_status',array('in'=>array('partial_payment')));
	   ...
	   $collection->addFieldToFilter('proof_id',array('neq'=>'0'));//04_03_2014
	...
	/************* Start for priority 07_03_2014 ********************/
	
	
	$collection->getSelect()->joinLeft(
		  array('pri' => 'vendor_priority'),
		  'main_table.priority = pri.entity_id',
		  array('pri.label')
	  );
...
function urlMaker()
   ...
   function printorder(orderid,itemid,proofid)
    {
	 ...
	 if (xmlhttp.readyState==4 && xmlhttp.status==200)
       ...
       form_key = \''.Mage::getSingleton('core/session')->getFormKey().'\';
       xmlhttp.open("POST","'.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'index.php/vendor/adminhtml_vendor/printload/?orderid="+orderid+"&itemid="+itemid+"&proofid="+proofid+"&form_key="+form_key+"&isAjax=true",true);
       xmlhttp.send();
...
$roleName = Mage::getModel('admin/roles')->load($roleId)->getRoleName();
   ...
   // if($roleName == 'Administrators')//11_03_2014
  //  {
   ...
   // }
	  ...
	  /* vendor printing detail*/
	  
	   $obj = Mage::getSingleton('vendor/printingstatus');	 
	   // var_dump($obj);
	   $_v_detail = $obj->getVendorPrintingDetail(); 	  
	   echo $_v_detail;
	  
	  $this->addColumn('product_sku', array(
          'header'    => Mage::helper('vendor')->__('Model SKU'),
          'align'     =>'left',
          'index'     => 'product_sku',
      ));
      
      $this->addColumn('qty', array(
          'header'    => Mage::helper('vendor')->__('Quantity'),
          'align'     =>'center',
          'index'     => 'qty',
      ));
	...
	//Zend_debug::dump($vendors);
	
	$this->addColumn('proof_approved', array(
          'header'    => Mage::helper('vendor')->__('Proof Approved '),
          'align'     =>'center',
		  'index'     =>'proof_approved',
		  'renderer'  =>'Artis_Vendor_Block_Adminhtml_Vendor_Renderer_ProofApproved',
		//  'type'	  => 'options',
		 // 'options'   =>array('Approved'=>'Approved', 'Disapproved'=>'Dis-Approved'),
          ));
	 $this->addColumn('file_visible', array(
          'header'    => Mage::helper('vendor')->__('File Status'),
          'align'     =>'center',
		  'index'	  =>'file_visible',
          'renderer'  =>'Artis_Vendor_Block_Adminhtml_Vendor_Renderer_Filevisible',
      ));	  
	 
	$this->addColumn('proof_date', array(
          'header'    => Mage::helper('vendor')->__('Proof Approve Date '),
          'align'     =>'center',		  
		  'index'     =>'proof_approve_date',
          ));
	 
	$this->addColumn('file_name', array(
          'header'    => Mage::helper('vendor')->__('Approved File '),
          'align'     =>'center',
		  'index'     =>'file_name',
          'renderer'  =>'Artis_Vendor_Block_Adminhtml_Vendor_Renderer_Fileimage',
      ));
	
	$this->addColumn('heigh_res_file', array(
          'header'    => Mage::helper('vendor')->__('H-Res File '),
          'align'     =>'center',
		  'index'     =>'heigh_res_file',
          'renderer'  =>'Artis_Vendor_Block_Adminhtml_Vendor_Renderer_HeighResFileimage',
      ));  
   
   $this->addColumn('time_since_order', array(
          'header'    => Mage::helper('vendor')->__('Printing Time Status '),
          'align'     =>'center',
		  'index'     =>'time_since_order',
		  'width'     =>'310px',
          'renderer'  =>'Artis_Vendor_Block_Adminhtml_Vendor_Renderer_TimeSinceOrder',
      ));  
