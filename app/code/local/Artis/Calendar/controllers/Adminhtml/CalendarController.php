...
class Artis_Calendar_Adminhtml_CalendarController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		...
		$sqlHoliday = $connectionRead->select()
				...
				->where('country_name=?', $_REQUEST['title']);
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			for($i=0;$i<=count($holidays->entry);$i++)
			{
				...
				if($event != '')
				{
					...
					$sqlHoliday = $connectionRead->select()
							->from($temptableHoliday, array('*'))
							->where("h_date = '".$date1."' AND event ='".$event."'");
					$chkHoliday = $connectionRead->fetchAll($sqlHoliday);
					...
					if($chkHoliday[0]['h_date'] == '')
					{
						...
						$data1 = array();
						$data1['country_name']= $_REQUEST['title'];
						$data1['h_date']=$date1;
						$data1['event']=$event;
						$data1['color']=$_REQUEST['color'];
						$connectionWrite->insert($temptableHoliday, $data1);
	...
	//	        }
			...
			try {
				...
				Mage::dispatchEvent('model_save_after', array('object'=>Mage::getSingleton('calendar/calendar')));
				mage::helper('AdminLogger')->updatelog($chkHoliday[0]['entity_id'],'Update Holiday List');
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				$sqlHoliday = $connectionRead->select()
					->from($temptableHoliday, array('*'))
					->where('calendar_id=?', $this->getRequest()->getParam('id'));
				$chkHoliday = $connectionRead->fetch($sqlHoliday);
				...
				$connectionWrite->beginTransaction();
				$condition = array($connectionWrite->quoteInto('country_name=?', $chkHoliday['title']));
				$connectionWrite->delete($temptableHoliday1, $condition);
				$connectionWrite->commit();
				/****************************************************************/
				//exit;
				...
				Mage::dispatchEvent('model_save_after', array('object'=>Mage::getSingleton('calendar/calendar')));
				mage::helper('AdminLogger')->updatelog($chkHoliday[0]['entity_id'],'Delete Holiday List');
	...
	$connectionWrite = Mage::getSingleton('core/resource')->getConnection('core_write');
        ...
        if(!is_array($calendarIds)) {
			...
			$connectionWrite->beginTransaction();
			$condition = array($connectionWrite->quoteInto('country_name=?', $chkHoliday[0]['title']));
			$connectionWrite->delete($temptableHoliday1, $condition);
			$connectionWrite->commit();
			/****************************************************************/
		...
		$chkHoliday = $connectionRead->fetchAll($sqlHoliday);
		   ...
		   try {
                  ...
                  // $sqlSaleOrder="SELECT count(increment_id) as ctct FROM ".$temptableSaleOrder." WHERE created_time between '".$start_time."' and '".$end_time."'";
                   $sqlSaleOrder = $connectionRead->select()
				->from($temptableSaleOrder, array('count(increment_id) as ctct'))
				->where("created_at between '".$start_time."' and '".$end_time."'");
		   try {
                       $chkSaleOrder = $connectionRead->fetchAll($sqlSaleOrder);
		   ...
		   try {
                   ...
                   $sqlSaleOrder = $connectionRead->select()
					->from($temptableSaleOrder, array('sum(total_paid) as ctct'))
					->where("created_at between '".$start_time."' and '".$end_time."'");
		   try {
                       $chkSaleOrder = $connectionRead->fetchAll($sqlSaleOrder);
                   ...
                   $sqlSaleOrder = $connectionRead->select()
					->from($temptableSaleOrder, array('count(entity_id) as ctct'))
					->where("status='pending' and created_at between '".$start_time."' and '".$end_time."'");
		   try {
                       $chkSaleOrder = $connectionRead->fetchAll($sqlSaleOrder);
                   ...
                   $sqlSaleOrder = $connectionRead->select()
					->from($temptableSaleOrder, array('sum(total_qty) as ctct'))
					->where("created_at between '".$start_time."' and '".$end_time."'");
		   try {
                       $chkSaleOrder = $connectionRead->fetchAll($sqlSaleOrder);
