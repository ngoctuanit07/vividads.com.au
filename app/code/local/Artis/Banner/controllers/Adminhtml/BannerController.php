...
class Artis_Banner_Adminhtml_BannerController extends Mage_Adminhtml_Controller_Action
{
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				try {	
					...
					// Any extention would work
					$uploader->setAllowedExtensions(array('jpg','jpeg','gif','png'));
					$result=$uploader->setAllowRenameFiles(true); // NEEDED FOR RENAME
					...
					$path = Mage::getBaseDir('media') . DS ;
					$uploader->save($path, $_FILES['filename']['name'] );
					$data['filename'] = $result->getuploadedFileName(); 
			...
			else if ((isset($_POST['filename']['delete'])) && ($_POST['filename']['delete'] == 1)){
			$pathToFile = Mage::getBaseDir('media'). $_POST['filename']['value'] ;
			...
			$data['filename'] = '';
				...
				$sql = "SELECT
							filename
						FROM
							`" . $resource->getTableName('banner') . "`
						WHERE
							`banner_id` = '{$id}'";
				$small_image = $resource->getConnection('core_read')->fetchOne($sql);
				if (empty($small_image) || is_null($small_image)) {
					$small_image = '';
				}
				$data['filename'] = $small_image;
			}
