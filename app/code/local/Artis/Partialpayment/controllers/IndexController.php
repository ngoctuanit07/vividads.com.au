...
/* Payment is being done from the front end 
  
  function paymentFrontAction
  @
  
  */
  
   public function paymentFrontendAction()
    {
	  extract($_REQUEST);
	...
	//       }
	       ...
	       $payemnt_type = $payment['payment_type'];
		 ...
		 if($payemnt_type =='anz_egate' || $payemnt_type =='anz_card'){
		   $type = 1;
		   $payment =null;
		   $param = $_REQUEST;
		   
		 //  var_dump($payemnt_type);
	     //  var_dump('main price='.$main_price.', due price='.$due_price.' amount ='.$amount );
	     //  exit;
	...
	//	   $err = 'Processing without credit card payment';
		   ...
		   if($err !=''){
		    ...
		    $sqlPaymentSystem = $connectionRead
								->select()
		   ...
		   // Mage::getSingleton('adminhtml')->getSession()->clear();
		    Mage::getSingleton('core/session')->addSuccess($this->__('The order has been saved.'));
	       } else{
		    Mage::getSingleton('core/session')->addError($err);
			}
	     ...
	     Mage::getSingleton('core/session')->addError($this->__('The partial payment amount are greater than order due amount.'));
	  }
	 ...
	 // $url1 = $this->getUrl("sales/order/view/order_id/".$orderid);
	 $url1= Mage::getUrl("sales/order/view/order_id/".$orderid);
	 
	// echo $url1; exit;
	 
	 $url1 = str_replace('p//s','p/front/s',$url1);
	//  Mage::log($url1); //To check if URL is correct (and it is correct)
	 // var_dump(Mage::getSingleton('core/session')->getMessages());	
	//  Mage::app()->getResponse()->setRedirect($url1)->sendResponse();
