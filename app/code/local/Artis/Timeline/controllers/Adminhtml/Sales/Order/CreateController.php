 ...
 */
class Artis_Partialpayment_Adminhtml_Sales_Order_CreateController extends Mage_Adminhtml_Controller_Action
{
      /**
                ...
                if($connectionWrite->isTableExists($temptableProduct))
                  {
                       // $sqlProduct="SELECT * FROM ".$temptableProduct." WHERE product_id = '".$ProductId."'";
                        $sqlProduct = $connectionRead->select()
                                            ...
                                            ->where('product_id=?',$ProductId);
                        $chkProduct = $connectionWrite->fetchAll($sqlProduct);
                  }
