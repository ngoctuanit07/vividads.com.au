 ...
 */
class BS_Carpet_Model_Sales_Quote extends Mage_Core_Model_Abstract
{
    //dfsdfsd
    ...
    public function addProductAdvanced( Mage_Catalog_Model_Product $product, $request = null, $processMode = null)
    {
		...
		$product = Mage::getModel('Mage_Catalog_Model_Product')->load($product->getEntity_id());
		...
		//	Zend_debug::dump($candidate);
		//	exit;
			// Child items can be sticked together only within their parent
            //   $stickWithinParent = $candidate->getParentProductId() ? $parentItem : null;
			 
			$stickWithinParent = $candidate->getParent_product_id() ? $parentItem : null;
            $candidate->setStickWithinParent($stickWithinParent);           		
			$item = $this->_addCatalogProduct($candidate, $candidate->getCartQty());
            
            ///28-3-2014 S
          //  $customPrice = $request->getPrice();
			$customPrice =null;
			if($product->getSpecial_price()){							
				$customPrice = $product->getSpecial_price();			
            ...
            ///10-4-2014 S
            $artworkImageVal=$request->getArtworkfile(); 
            $cusNotes=$request->getCustomernotes();
            $dimension=$request->getDimension();
            $item->setArtworkfile($artworkImageVal);
            $item->setCustomernotes($cusNotes);
		 ...
		 //  if ($parentItem && $candidate->getParentProductId()) {
		    if ($parentItem && $candidate->getParent_product_id()) {
