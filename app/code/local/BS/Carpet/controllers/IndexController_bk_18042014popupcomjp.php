	...
	foreach($optArr as $optionKey => $optionVal) {
	    ...
	    if($optionVal->getIsDependent() == 0){ //$style ='style ="display:block;"';} else{$style ='style ="display:none;"';
    		...
    		$str .='<input class="hideoption" type="hidden" value="0" id="hideopt_'.$optionKey.'"/><dl class="clearfix" id="opt__'.$optionKey.'" '.$style.'><dt><label><b>'.$optionVal->getTitle().'</b></label></dt><dd>
				<select class="textfield" id="select_'.$optionKey.'" onchange="getchildrenpop('.$optionKey.',this.value)"><option value="" price="0">-- Please Select --</option>';
...
public function getchildoptpopAction(){
	...
	if($row['dependent_ids'] != ''){
		...
		$retStr .= '<input class="hideoption" type="hidden" value="0" id="hideopt_'.$k.'"/><dl id="opt__'.$k.'" class="clearfix"><dt><label><b>'.$row2['title'].'</b></label></dt><dd><select class="textfield" id="select_'.$k.'"><option price="0" value="">-- Please Select --</option>';
