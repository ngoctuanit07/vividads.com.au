    ...
    public function addCommentAction() {                
        ...
        if ($order = $this->_initOrder()) {
            try {
                ...
                /******************* Start by dev to add comment in invoice and shipment ************************/
                $comment = trim(strip_tags($data['comment']));
			  ...
			  // $history  = Mage::getModel('sales/order_status_history')->load();
			   
			    $order_id = $this->getRequest()->getParam('order_id');
				//var_dump($order_id);
				//exit;
				$temptableOrderHistory  = Mage::getSingleton('core/resource')->getTableName('sales_flat_order_status_history');
				$temptableOrder 		= Mage::getSingleton('core/resource')->getTableName('sales_flat_order');
				
				$sqlmessage = 'update '.$temptableOrderHistory.' set `readstatus`=0 WHERE `parent_id`='.$order_id;
				Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlmessage);
		
		
				$sqlordermessage = 'update '.$temptableOrder.' set `readstatus`=0 WHERE `entity_id`='.$order_id;
				Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlordermessage);
				
			   
			   
			   
			    $order->sendOrderUpdateEmail($notify, $comment, $order);
