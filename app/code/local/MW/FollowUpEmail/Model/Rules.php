...
//    }
...

    {
        ...
        $templateName = substr($templateId,
            ...
            false !== ($pos = strpos($templateId, MW_FollowUpEmail_Model_System_Config_Emailtemplate::TEMPLATE_SOURCE_SEPARATOR))
    ...
    {      
		...
		if($data['testemail']['test_customer_name'] != ""){		
			...
			if($customer != null){				
		        ...
		        $select = $read->select()
					...
					->where('q.customer_email = ?',$data['testemail']['test_customer_name'])           
		...
		if($cart != null) $codeCart = MW_FollowUpEmail_Helper_Data::encryptCode($data['testemail']['test_customer_name'],'cart',0);
		...
		foreach ($emailChain as $emailChainItem) {
			...
			if($templateId == $emailChainItem['TEMPLATE_ID']) continue;
