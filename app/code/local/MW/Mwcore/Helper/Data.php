<?php
class MW_Mwcore_Helper_Data extends Mage_Core_Helper_Abstract
{
   // $module_special_name is module when click extend trial module
	...
	{	
    	...
    	if(count($modules_install)>0)
    	{
			 ...
			 Mage::getModel('core/config')->saveConfig('mwcore/allextension/activated',$this->all_active);
   ...
   function startTrial($module_name,$type)
	...
	{
		return 'mw_'.md5('mwcore/'.strtolower($modulename));
	}
	...
	function getModuleConfig($modulename)
	{
		...
		$key_config="";
		$modules_company = Mage::getStoreConfig('mwcore/extensions'); 
	...
	public function showTimeCheckStatus($modulename,$position="")
	{
		...
		$url = Mage::helper('adminhtml')->getUrl('mwcore/adminhtml_index/trial/module/'.$encmodule); // Mage::getUrl('mcore/index/trial/module/'.$encmodule);
	...
	{	
		...
		if($client_data['type_site'] !=Mage::getStoreConfig('mwcore/type1'))
		{
			...
			{							
			    ...
			    $mod_infs[1] = Mage::getStoreConfig('mwcore/decrt');
		...
		{		
		  if(!$this->checkSatisfies($client_data,$result))
			...
			{				
			    ...
			    $mod_infs[1] = Mage::getStoreConfig('mwcore/decrtdev');
	    		...
	    		{	 	
					if($module!="" ) 
					{
					    if (class_exists('SoapClient'))
				    		 {
				    		 	if(Mage::getStoreConfig('mwcore/errorSoap')==1)
			    		 		Mage::getModel('core/config')->saveConfig('mwcore/errorSoap',0);
								Mage::getConfig()->reinit();
	...
	{	
		$curr_dis = Mage::getModel("mwcore/notification")->getCollection();		
		...
		if(count($server_notification)>0)
		{
			...
			{
				$notification = Mage::getModel('mwcore/notification')->load($row['notification_id']);
				  ...
				  if (class_exists('SoapClient'))
				    	{				    		 	
							...
							if(is_array($result) && count($result)>0)
							{
								...
								{
										$noti_inf = Mage::getModel('mwcore/notification');
										$noti_inf->setData($notification);									
										$noti_inf->setStatus(0); // is normal
										$noti_inf->save();								
								}
							}
						}
	...
	{   					
		try {
					$resource = Mage::getSingleton('core/resource');
	    			$readConnection = $resource->getConnection('core_read');
	    			$tableName = $resource->getTableName('mwcore/notification');
	...
	public function insertNotification($type,$extension_key)
	{
		...
		if(!$this->existNotification($type,$extension_key))
		{
			...
			$notification_insert = Mage::getModel('mwcore/notification');
