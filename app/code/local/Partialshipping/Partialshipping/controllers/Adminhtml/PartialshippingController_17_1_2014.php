	...
	public function partshipformAction(){
		...
		//if( $qtyShipped < $totalqtyOrdered || $qtyShipped != 0){
			...
			if($shipAd !='' && $shipMd !='' && $_REQUEST['boxstr'] != '' ){
				...
				$addressLoadId = Mage::getModel('customer/address')->load($shipAd);
				$Name = $addressLoadId['firstname'].' '.$addressLoadId['lastname'];
				...
				/********************* Start upload file to ft server *****************************/
					...
					$date = date('dMY');
					do{
						$file = Mage::getBaseDir('media') . DS ."shiplabel" . DS ."ET-".$date.$f.'.txt';
						$remote_file = '/outbox/'."ET-".$date.$f.'.txt';
						$f++;
				...
				foreach($box as $q=>$boxvalue)
				{
					...
					for($j ; $j<=$boxdata_item['box'] ; $j++)
					{
						...
						$itemno = "001" . $conNote . (str_pad($j,3,"0",STR_PAD_LEFT));
						//$itemno =  $key;	
						//$pdf->AddPage("P","A4");		
$tbl[] = <<<EOD
...
if($j%7 == 0)
{
	...
	$data .="
	H".(str_pad($manifat_number,10,"0",STR_PAD_LEFT)).$conNoteNumber."   ".(str_pad((round($j/7)),3,"0",STR_PAD_LEFT)).$allitem;
	$l++;
	$allitem = '';
...
elseif($j == $boxdata_item['box'])
{
	$allitem .= $conNote . (str_pad($j,3,"0",STR_PAD_LEFT));
	$data .="
	H".(str_pad($manifat_number,10,"0",STR_PAD_LEFT)).$conNoteNumber."   ".(str_pad((round($j/7)+1),3,"0",STR_PAD_LEFT)).$allitem;
	$l++;
	$allitem = '';
}
else
...
//	 $extra=true;
	...
	////10-12-2013 modified on 12-12-2013 SOC
	public function getconfirmboxAction(){
		extract($_REQUEST);
		$proArr=explode("**",$pId);
		$totweight=0;
		foreach($proArr as $pi){
		...
		$str='<form id="cnfrm" name="cnfrm" action="'.$urlact.'" method="post">
			...
			<tr>
			<td colspan="2"> 
				<table id="tabl_row_'.$j.'">
					<tr class="headings">
					<th>Boxes</th><th>Qty</th><th>Lenght</th><th>Width</th><th>Height</th><th>weight</th><th>Title</th><th>Action</th>
					</tr>
					<tr id="row_'.$i.'">
					<td><input type="text" name="confirmbox[]" id="confirmbox" style="width : 80px"/></td>
					<td><input type="text" name="confirmqty[]" id="confirmqty" style="width : 80px"/></td>
					<td><input type="text" name="confirmlength[]" id="confirmlength" style="width : 80px"/></td>
					<td><input type="text" name="confirmwidth[]" id="confirmwidth" style="width : 80px"/></td>
					<td><input type="text" name="confirmheight[]" id="confirmheight" style="width : 80px"/></td>
					<td><input type="text" name="confirmweight[]" id="confirmheight" style="width : 80px"/></td>
					<td><input type="text" name="confirmtitle[]" id="confirmtitle" style="width : 80px"/> </td>
			...
			<span onclick="add_more('.$j.');" style="cursor:pointer;float: right;" title="Add More" class="addanother">Add More</span></td></tr>
			<tr>
	...
	if ($destCountry == "AU") {
		$requestXml='<?xml version="1.0"?>
			...
			foreach($allItems as $item){
				...
				if($product->getWeight()) $weight = $product->getWeight(); else $weight = 100;
				if($product->getLength()) $length = $product->getLength(); else $length = 100;
				if($product->getWidth())  $width = $product->getWidth(); else $width = 100;
				if($product->getHeight()) $height = $item->getQty(); else $height = 100;
			...
			if($array['ratedTransitTimeResponse']['brokenRules'] && !$array['ratedTransitTimeResponse']['ratedProducts']){
				...
				foreach($ratedArray as $key=>$ratedProduct){
					...
					$val = $mt."__".$ratedProduct['quote']['price'];
					$mt1=$mt." ".$rate;
