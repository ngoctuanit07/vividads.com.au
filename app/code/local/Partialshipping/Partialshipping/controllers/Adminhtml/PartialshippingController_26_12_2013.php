...
class Partialshipping_Partialshipping_Adminhtml_PartialshippingController extends Mage_Adminhtml_Controller_action
{
	...
	if ($destCountry == "AU") {
		$requestXml='<?xml version="1.0"?>
			...
			$ratedArray = $array['ratedTransitTimeResponse']['ratedProducts'];
			$res='';
			foreach($ratedArray as $ratedProduct){
				
				$mt = $ratedProduct['product']['description'];
				$res .= '<input name="shipping_method_tntl" type="radio" value="" id="" class="radio" onclick="slmthd(this.value);"/>  '.$mt.'</br>';
