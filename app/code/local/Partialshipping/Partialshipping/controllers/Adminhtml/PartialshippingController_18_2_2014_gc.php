	...
	public function partshipformAction(){
		...
		//if( $qtyShipped < $totalqtyOrdered || $qtyShipped != 0){
			...
			if($shipAd !='' && $shipMd !='' && $_REQUEST['boxstr'] != '' ){
				...
				$postCode = strtoupper($addressLoadId['postcode']);
				$city = strtoupper($addressLoadId['city']);
				$firstName = strtoupper($addressLoadId['firstname']);
				$lastName = strtoupper($addressLoadId['lastname']);
				$region = strtoupper($addressLoadId['region']);
				$street = strtoupper($addressLoadId['street']);
				$telephone = strtoupper($addressLoadId['telephone']);
				$company = strtoupper($addressLoadId['company']);
				
				$regionCollection = Mage::getModel('directory/region_api')->items($addressLoadId['country_id']);
				//print_r($regionCollection);exit;
				
				foreach($regionCollection as $r)
				{
					if($r['name'] == $addressLoadId['region'])
				...
				/********************* Start upload file to ft server *****************************/
					...
					$date1 = date('dMY');
					do{
						$file = Mage::getBaseDir('media') . DS ."shiplabel" . DS ."ET-".$date1.$f.'.dat';
						$remote_file = '/outbox/'."ET-".$date1.$f.'.dat';
						$f++;
					...
					if(($f-1) == 0)
					$data3['et_file']= "ET-".$date1.'.dat';
					else
					$data3['et_file']= "ET-".$date1.$f.'.dat';
					...
					C".(str_pad($manifat_number,10,"0",STR_PAD_LEFT)).$conNoteNumber."   ".(str_pad($counter,3,"0",STR_PAD_LEFT))."                         ".$firstName."    ".$lastName."     ".$company."      ".$street.'      '.$city."     ".$region_code."        ".$postcode."  ".$firstName."    ".$lastName."  ".$telephone."  ".date("dmY").$tnt_methos[$method[0]]."  0S0000000000 ";
					$l++;
				...
				foreach($box as $q=>$boxvalue)
				{
					...
					for($j ; $j<=$boxdata_item['box'] ; $j++)
					{
						...
						//$pdf->AddPage("P","A4");		
$tbl[] = <<<EOD
<table width="240" height="200" border="0" align="center" cellpadding="0" cellspacing="0">
<tr>
...
<table border="0" cellspacing="0" cellpadding="0" align="right">
  ...
  <tr>
	...
	<td style="font-size:14;font-weight:bold;" width="220" height="80">{$firstName} {$lastName}<br/> {$street}<br/> {$city} {$region_code}<br/></td>
      </tr>
    </table>
      </td>
  </tr>
...
<tr> <td colspan="2"></td></tr>
  <tr>
	...
	<tr>
	  <td  width="50%" valign="top" height="75" style="line-height:0.9;border:#000 2px solid;font-size:7;" align="left"><strong>CN:{$conNoteNumber}<br>Itm:{$itemno}<br>{$count} of {$total}<br>TO:</strong><BR>{$firstName} {$lastName}<br/> {$company}<br/> {$street}<br/> {$city}<br/> {$region_code} {$postCode}</td>
	  <td width="50%" valign="top" align="left" style="line-height:0.9;border-bottom:#000 2px solid; border-right:#000 2px solid;font-size:6;border-top:#000 2px solid"><strong>Service Title here<br>Con Note Wt.: {$boxdata_item['weight']} Kg.<br><br>FROM:</span><BR>{$vivid['company']}<br/> {$vivid['address1']}<br/> {$vivid['city']}<br/> {$vivid['state']} {$vivid['zip']}</td></tr>
			...
			$counter++;
				...
				foreach($_REQUEST['shipment'] as $key=>$val){
					...
					foreach($val as $k=>$qty){
						...
						$Insert = "INSERT INTO ".$tableName." SET parent_id = '".$new_ship_id."', price = '".$row1['price']."', weight = '".$row1['weight']."', qty = '".$qty."', product_id = '".$k."', order_item_id = '".$row1['item_id']."', name = '".$row1['name']."', sku = '".$row1['sku']."'; commit;";
...
Z".(str_pad($manifat_number,20,"0",STR_PAD_LEFT)).(str_pad($l,5,"0",STR_PAD_LEFT));
		...
		try
		{
			...
			$Insert = "INSERT INTO ".$tableNameTrack." SET parent_id = '".$new_ship_id."',  weight = '".$row1['weight']."', qty = '".$qty."', order_id = '".$ordId."', track_number = '30015', title = '".$title."', carrier_code = '".$sel_metd2[0]."', created_at= NOW(), updated_at = NOW(); commit; ";
			$connectionWrite->query($Insert);
			//$connectionWrite->commit();
			//exit;
		}
...
//	 $extra=true;
			...
			//$pdf->Output('example_001.pdf', 'I');		
				...
				//
				//if(($f-1) == 0)
				//$data3['et_file']= "ET-".$date.'.dat';
				//else
				//$data3['et_file']= "ET-".$date.$f.'.dat';
				//
				//$connectionWrite->insert($tableName4, $data3);
				//$connectionWrite->commit();
				//
	...
	public function getconfirmboxAction(){
		...
		$str='<form id="cnfrm" name="cnfrm" action="'.$urlact.'" method="post">
			...
			<td>Total Volum : <input type="text" name="confirmtotalvolum" id="confirmtotalvolum" value="'.count($row).'"/></td>
	...
	///26-12-2013
	// modified on 13-2-2014
	public function gettntintlAction(){extract($_REQUEST);
		///21-1-2013  S
		$order = Mage::getModel('sales/order')->load($ordId);
		$pos1 = stripos($addrsid, '_S');
		if($pos1 != false){
