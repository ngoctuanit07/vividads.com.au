...
class Partialshipping_Partialshipping_Adminhtml_PartialshippingController extends Mage_Adminhtml_Controller_Action
{
	...
	public function partshipformAction(){
		...
		try
		{
			...
			$Insert = "INSERT INTO ".$tableNameTrack." SET parent_id = '".$shipId."',  weight = '".$row1['weight']."', qty = '".$qty."', order_id = '".$ordId."', track_number = '".$conNoteNumber."', title = '".$title."', carrier_code = '".$sel_metd2[0]."', created_at= NOW(), updated_at = NOW(); commit; ";
		...
		/********************* Start for shipment track **************************/
			...
			if($tnt == 1){
				...
				if($country_code == 'AU'){
					$etDate = date("dMY", strtotime($shipdate));  ///13-3-2014
					$res = Mage::getModel('partialshipping/partialshipping')->getTNTlebal($box,$conNoteNumber,$addressLoadId,$_REQUEST,$method,$shipId,$invIncrementID,$order);
					Mage::getModel('partialshipping/partialshipping')->getTNTet($box,$conNoteNumber,$addressLoadId,$_REQUEST,$method,$shipId,$invIncrementID,$order,$addr,$etDate); ///13-3-2014
					//print_r($res);
					//echo "RES : ".$res;
					//exit;
				}else{
					//$conNoteNumber=str_pad($lastId,9,"0",STR_PAD_LEFT);
					$conNoteNumber="VVD".(str_pad($lastId,9,"0",STR_PAD_LEFT)); ///18-3-2014
	...
	///29-11-2013  ///modified on 6-3-2014
	public function deletepartshipAction(){
		...
		if($pos1 != false) {
			...
			///delete from item
			$connectionWrite->beginTransaction();
			$condition1 = array($connectionWrite->quoteInto("parent_id = '".$parentId."' AND shipment_grid_id = '".$partShipID."'"));
			...
			////6-3-2014 S
			//$selS = $connectionRead->select()->from($tableName, array('*'))->where('order_id=?',$orderId);
			$selS = $connectionRead->select()->from($tableName1, array('*'))->where("order_id = '".$orderId."' AND increment_id = '".$a."'");
			$roS = $connectionRead->fetchRow($selS);
			$allShipped=$roS['allshipped'];
			
			if($allShipped == 1){
				$connectionWrite->beginTransaction();
				$data=array();
				$data['allshipped']= 0;
				$where = $connectionWrite->quoteInto("order_id = '".$orderId."' AND increment_id = '".$a."'");
				$connectionWrite->update($tableName1, $data, $where);
				$connectionWrite->commit();	
	...
	public function gettntlocalmethodAction(){
		...
		//$date = '2014-10-10';
		$nDate = now(); ///11-3-2014
		$sdate= explode(" ",$shipDate);	 ///21-4-2014	
		$nmdate = str_replace(" ","T",$nDate);
		if ($destCountry == "AU") {
			$requestXml='<?xml version="1.0"?>
				<enquiry xmlns="http://www.tntexpress.com.au">
				  <ratedTransitTimeEnquiry>
					<cutOffTimeEnquiry>
					  ...
					  <shippingDate>'.trim($sdate[0]).'</shippingDate>
					  <userCurrentLocalDateTime>
						2007-11-05T10:00:00
					  </userCurrentLocalDateTime>
					  <dangerousGoods>
				...
				if($array['ratedTransitTimeResponse']['brokenRules'] && !$array['ratedTransitTimeResponse']['ratedProducts']){
					...
					$valh = "TNT ".$mth."__".number_format($chge,2)."__".$codeh;
						$mth1=$mth." ".$rte;
					...
					foreach($ratedArray as $key=>$ratedProduct){
						if(is_numeric($key)){
						$mt = $ratedProduct['product']['description'];
						$code = $ratedProduct['product']['code'];
						///17-2-2014 GC S
						$charge = $ratedProduct['quote']['price'];
						$margin = Mage::getStoreConfig('carriers/excellence/profit_margin')/100;
						$charge = $charge + ($charge*$margin);
						...
						$val = "TNT ".$mt."__".number_format($charge,2)."__".$code;
						$mt1=$mt." ".$rate;
						$res .= '<input name="shipping_method_tntl" type="radio" value="'.$val.'" id="" class="radio" onclick="slmthd(this.value,'.$shipId.');"/>  '.$mt1.'</br>';
						}
					}
	...
	public function partshipformnewAction(){
		...
		if($flg > 0){
			...
			$t=time();
			$created_at = now(); //date("Y-m-d H:i:s", $t);
			$date_post = strtotime($order->getCreatedAtDate()); 
			$Ordtime=date('Y-m-d H:i:s',$date_post );
			...
			if($qtyShipped > 0){
				...
				if($order->getCreatedAt()) $data['order_created_at']=$order->getCreatedAt(); ///12-3-2014
