		...
		if( $qtyS > $qtny){
			...
			if($shipAd !='' && $shipMd !='' && $_REQUEST['box'] != '' ){
				...
				foreach($box as $q=>$boxvalue)
				{
					...
					///for($j ; $j<=$boxdata_item['box'] ; $j++){ ////24-2-2014
...
<table border="0" cellspacing="0" cellpadding="0" align="right">
  ...
  <tr>
	...
	<td align="center" style="font-size:9">{$j} of {$total}</td>
  ...
  <tr>
	...
	<td style="font-size:14;font-weight:bold;" width="220" height="80">{$firstName} {$lastName} {$cmp},<br/> {$street} , {$city}, {$region}<br/></td>
      </tr>
    </table>
      </td>
  </tr>
...
<tr> <td colspan="2"></td></tr>
  <tr>
	...
	<tr>
	  <td  width="50%" valign="top" height="75" style="line-height:0.9;border:#000 2px solid;font-size:7;" align="left"><strong>CN:{$conNoteNumber}<br>Itm:{$itemno}<br>{$q} of {$total}<br>TO:</strong><BR>{$firstName} {$lastName} {$cmp1}, {$street}, {$city}, {$region} {$postcode}</td>
...
if($counter==1){
	...
	$order_items=$order->getAllItems();																	
		foreach($order_items as $orderDetails){
			...
			//if($product->getThumbnailUrl())$thumbnail = $product->getThumbnailUrl();												
