...
class MDN_AdminLogger_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function updatelog($id,$description)
	{
		...
		$select = $connectionRead->select()
			->from($temptableLog, array('*'))
			->order('al_id DESC')
			->limit(1);
		$logfetch = $connectionRead->fetchRow($select);
		
		
		$statement = str_replace('id','id '.$id,$logfetch['al_object_description']);
		
		$connectionWrite->beginTransaction();
		$data = array();
		$data['al_object_id']= $id;
		$data['al_object_description']= $statement;
		$data['al_description']= $description;
		$data['al_custom_type']= 1;
		$where = $connectionWrite->quoteInto('al_id =?', $logfetch['al_id']);
		$connectionWrite->update($temptableLog, $data, $where);
