...
class Ace_Anz_Model_Egate extends Mage_Payment_Model_Method_Cc
{
    ...
    ////modified on 25-4-2014 gc
    public function _call($type, Varien_Object $payment=null, $param=null)
    {
	$postRequestData = '';
        $amp = '';
	$request = array();
       ...
       if(count($param) > 0){
	    ...
	    //exit;
	    $Order = Mage::getModel('sales/order')->load($param['orderid']);
	    $amount = $param['amount']*100;
	    $date_expiry = substr($param['payment']['cc_exp_year'], 2, 2) . str_pad($param['payment']['cc_exp_month'], 2, '0', STR_PAD_LEFT);
	    $request['vpc_Version'] = '1';
	    $request['vpc_MerchTxnRef'] = $Order->getIncrementId().'-'.'055253';  ///$payment->getCcTransId();
	    ...
	    $request['vpc_OrderInfo'] = $Order->getIncrementId();
	    $request['vpc_CardNum'] = htmlentities($param['payment']['cc_number']);
	    $request['vpc_CardExp'] = htmlentities($date_expiry);
	    $request['vpc_CardSecurityCode'] = htmlentities($param['payment']['cc_cid']);
