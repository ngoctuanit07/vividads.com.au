<?php
include_once('MDN/AdvancedStock/MiscController.php');
    ...
    public function Savepayment($order_id=0, $payment_validated=0) {
        ...
        if($value == '1' && $order_validated !=1 )
        {
           // echo $value;
            ...
            $connectionWrite = Mage::getSingleton('core/resource')->getConnection('core_write');
		     ...
		     //$_order->setTotalDue(1);
			// $_order->getTotalPaid(0);
	    ...
	    //End 03_03_2014
			...
			if($order->getTotalPaid() > 0)
						...
						$order_placed_date =  $created_date;
                        $artwork_date = $this->gettimelinedate2($chkTimeline[0]['artwork_day'],$created_date,$chkTimeline[0]['sunday_artwork'],$chkTimeline[0]['holiday_artwork']);
                        $proof_date = $this->gettimelinedate2($chkTimeline[0]['proof_day'],$created_date,$chkTimeline[0]['sunday_proof'],$chkTimeline[0]['holiday_proof']);
                        $production_start_date = $this->gettimelinedate2($chkTimeline[0]['production_day'],$created_date,$chkTimeline[0]['sunday_production'],$chkTimeline[0]['holiday_production']);
                        $shipping_date = $this->gettimelinedate2($chkTimeline[0]['shipping_day'],$created_date,$chkTimeline[0]['sunday_shipping'],$chkTimeline[0]['holiday_shipping']);
                        $delivery_date = $this->gettimelinedate2($chkTimeline[0]['delivary_day'],$created_date,$chkTimeline[0]['sunday_delivary'],$chkTimeline[0]['holiday_delivary']);
   ...
   public function pricechangeAction()
   {
    ...
    public function savebillingAction(){
	   ...
	   if($connectionWrite->isTableExists($table_billing))
	    ...
	    /***************************** Add custom function ***********************************/
     public function isweekend2($date){
     $date = strtotime($date);
     $date = date("l", $date);
     $date = strtolower($date);
     if($date == "sunday"){
    ...
    public function gettimelinedate2($day_delay,$created_date,$sunday,$holiday)
    {
        $connectionRead = Mage::getSingleton('core/resource')->getConnection('core_read');
	    $connectionWrite = Mage::getSingleton('core/resource')->getConnection('core_write');
        ...
        else
        {
            if($sunday == 1)
            {
                ...
                $d = $this->isweekend2($artwork_date);
                if($holiday == 1)
                {
                    while($flag == 0)
                    {
            ...
            else if($holiday == 1)
            {
                ...
                while($flag == 0)
                {
                   ...
                   // $sqlHoliday="SELECT * FROM ".$temptableHoliday." WHERE h_date = '".$artwork_date."' ";
                    ...
                    if(count($chkHoliday) > 0 or ($sunday == 1 and $this->isweekend2($artwork_date) == 1))
                    {
                        $d++;
                    }
                    else
