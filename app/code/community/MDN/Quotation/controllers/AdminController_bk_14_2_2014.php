    ...
    public function createorderAction() {
		...
		// exit('no shipping address');
                        ...
                        /********************** Start for bundle product 12_02_2014 ********************************/
                            ...
                            foreach($chkItem as $bItem)
                            {
                                ...
                                $orderItem = Mage::getModel('sales/order_item')
                                        ->setStoreId($storeId)
                                        ->setQuoteItemId(0)
                                        ->setQuoteParentItemId($last_item_id)
		    ...
		    $data2['qty'] = $result[0]['quantity'];
                ...
                $temptableOrderAddress = Mage::getSingleton('core/resource')->getTableName('sales_flat_order_address');
                ...
                $connectionWrite->beginTransaction();
                $datab = array();
                $datab['parent_id']= $order->getId();
                if($chkBill['region_id'] != '')
                $datab['region_id'] = $chkBill['region_id'];
                
                $datab['customer_id'] = $customerId;
                if($chkBill['region'] != '') 
                $datab['region'] = $chkBill['region'];
                
                if($chkBill['postcode'] != '')
                $datab['postcode'] = $chkBill['postcode'];
                
                if($chkBill['lastname'] != '')
                $datab['lastname'] = $chkBill['lastname'];
                
                if($chkBill['street1'] != '' or $chkBill['street2'] != '')
                $datab['street'] = $chkBill['street1'].' , '.$chkBill['street2'];
                
                if($chkBill['city'] != '')
                $datab['city'] = $chkBill['city']; 
                $datab['email'] = $customer->getEmail();
                
                if($chkBill['telephone'] != '')
                $datab['telephone']= $chkBill['telephone'];
                
                if($chkBill['country_id'] != '')
                $datab['country_id']= $chkBill['country_id'];
                
                if($chkBill['firstname'] != '')
                $datab['firstname']= $chkBill['firstname'];
                
                if($chkBill['company'] != '')
                $datab['company']= $chkBill['company'];
                
                $datab['address_type']= 'billing';
                $connectionWrite->insert($temptableOrderAddress, $datab);
                $connectionWrite->commit();
                ...
                $connectionWrite->beginTransaction();
                $datas = array();
                $datas['parent_id']= $order->getId();
                
                if($chkShip['region_id'] != '')
                $datas['region_id'] = $chkShip['region_id'];
                $datas['customer_id'] = $customerId;
                
                if($chkShip['region'] != '')
                $datas['region'] = $chkShip['region'];
                
                if($chkShip['postcode'] != '')
                $datas['postcode'] = $chkShip['postcode'];
                
                if($chkShip['lastname'] != '')
                $datas['lastname'] = $chkShip['lastname'];
                
                if($chkShip['street1'] != '' or $chkShip['street2'] != '')
                $datas['street'] = $chkShip['street1'].' , '.$chkShip['street2'];
                
                if($chkShip['city'] != '')
                $datas['city'] = $chkShip['city']; 
                $datas['email'] = $customer->getEmail();
                
                if($chkShip['telephone'] != '')
                $datas['telephone']= $chkShip['telephone'];
                
                if($chkShip['country_id'] != '')
                $datas['country_id']= $chkShip['country_id'];
                
                if($chkShip['firstname'] != '')
                $datas['firstname']= $chkShip['firstname'];
                
                if($chkShip['company'] != '')
                $datas['company']= $chkShip['company'];
                
                $datas['address_type']= 'shipping';
                $connectionWrite->insert($temptableOrderAddress, $datas);
                $connectionWrite->commit();
                
                $connectionWrite->beginTransaction();
                $condition = array($connectionWrite->quoteInto('parent_id= "'.$order->getId().'" AND customer_id = "" ', '5'));
                $connectionWrite->delete($temptableOrderAddress, $condition);
                $connectionWrite->commit();
