...
class MDN_Quotation_Model_QuotationPdf extends MDN_Quotation_Model_Pdfhelper {
	...
	if($fetchSystem['telephone'] != '')
        ...
        $page->setFillColor(Zend_Pdf_Color_Html::color('#DCDCDC'));
        $page->drawRectangle(300, $this->y+25, 550, $this->y, Zend_Pdf_Page::SHAPE_DRAW_FILL);
        $page->setFillColor(Zend_Pdf_Color_Html::color('#000000'));
        $this->drawTextInBlock($page, 'Method', 300, $this->y+10, 500,$this->y+20,'l');
        
        $page->setFillColor(Zend_Pdf_Color_Html::color('#FFFFFF'));
        $page->setLineColor(Zend_Pdf_Color_Html::color('#DCDCDC'));
        $page->drawRectangle(301, $this->y-25, 549, $this->y);
        
        $carrierTitle =  end(explode('_',$quote->getShippingMethod()));
        $ship = Mage::getStoreConfig('carriers/'.$carrierTitle.'/title');
        
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, $ship, 304, $this->y-20, 549,$this->y,'l');
        ...
        $page->setFillColor(Zend_Pdf_Color_Html::color('#FFFFFF'));
        $page->setLineColor(Zend_Pdf_Color_Html::color('#DCDCDC'));
        $page->drawRectangle(21, $this->y-165, 269, $this->y-20);
	...
	$chkPlanning = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchALL($sqlPlanning);
        ...
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Order Placed By: '.date('d/m/Y',strtotime($chkPlanning[0]['order_placed_date'])), 24, $this->y-55, 269,$this->y-20,'l');
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Artwork Submitted By: '.date('d/m/Y',strtotime($chkPlanning[0]['artwork_date'])), 24, $this->y-75, 269,$this->y-20,'l');
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Proof Approval Date: '.date('d/m/Y',strtotime($chkPlanning[0]['proof_date'])), 24, $this->y-95, 269,$this->y-20,'l');
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Production Start: '.date('d/m/Y',strtotime($chkPlanning[0]['start_date'])), 24, $this->y-115, 269,$this->y-20,'l');
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Shipping On: '.date('d/m/Y',strtotime($chkPlanning[0]['shipping_date'])), 24, $this->y-135, 269,$this->y-20,'l');
        $page->setFillColor(Zend_Pdf_Color_Html::color('#A9A7A7'));
        $this->drawTextInBlock($page, 'Delivery On: '.date('d/m/Y',strtotime($chkPlanning[0]['delivery_date'])), 24, $this->y-155, 269,$this->y-20,'l');
    ...
    public function drawTableHeader(&$page) {
        ...
        $page->setFillColor(Zend_Pdf_Color_Html::color('#000000'));
        $page->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA), 10);
        $page->drawText(Mage::helper('quotation')->__('Description'), 15, $this->y, 'UTF-8');
    ...
    protected function drawTotals($page, $quote) {
       ...
       // $this->y -= 20;
        ...
        $page->drawText(Mage::helper('quotation')->__('Subtotal:'), $this->_PAGE_WIDTH / 2, $this->y, 'UTF-8');
        $this->drawTextInBlock($page, $quote->FormatPrice($quote->getPriceHt()), $this->_PAGE_WIDTH / 2 + 40, $this->y, $this->_PAGE_WIDTH / 2, 40, 'r');
