...
class MDN_Quotation_Model_Quotationitem extends Mage_Core_Model_Abstract {
	...
	public function getAllItems($quotation_id=null){
		 $_items = $this->getCollection()
		 				->addFieldToFilter('quotation_id',$quotation_id)
		 					;
		 ///fetch Items
		 $bundle_products = new Varien_Data_Collection();
		  
		 foreach($_items as $_item){
			 $item_type =  $_item->getProduct_type();
			 
				 $product = Mage::getModel('catalog/product')->load($_item->getProduct_id());
				 $bundle_products->addItem($_item);
	...
	///get bundle children items of a item
	
	public function getItemBundleItems($_item=null){
		 
		 $bundle_products = new Varien_Data_Collection();
		 $product = $_item;
			$item_type = $product->getType_id(); 		 
			// var_dump($item_type);
			// var_dump($_item);
			 if($item_type =='bundle'){
				$product = Mage::getModel('catalog/product')->load($_item->getEntity_id());
				$collection = $product->getTypeInstance(true)
    						 ->getSelectionsCollection(
        							$product->getTypeInstance(true)
                			 ->getOptionsIds($product), $product)
					...
					foreach ($collection as $_bitem) {
						# $item->product_id has the product id.						
						  $bundle_products->addItem($_bitem);
					}
			 }
	...
	///function getBundledChildrenProducts()
	
	public function getBundledChildrenProduct($_product=null){
