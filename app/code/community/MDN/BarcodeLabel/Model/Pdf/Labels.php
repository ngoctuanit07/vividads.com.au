...
class MDN_BarcodeLabel_Model_Pdf_Labels extends Mage_Sales_Model_Order_Pdf_Abstract {
    ...
    /*************************************************************************************************/
     ...
     public function getPdfOrder($products = array()) {
        ...
        foreach ($products as $productId => $qty) {
            for ($i = 0; $i < $qty; $i++) {
		$all = explode('-',$productId);
                $this->printLabelOrder($all[0],$all[1], $qty);
            }
        }
     ...
     *
     */
    public function printLabelOrder($productId,$orderId, $qty) {
        ...
        // get image
        $img = Mage::helper('BarcodeLabel/Label')->getImageOrder($orderId,$productId);
