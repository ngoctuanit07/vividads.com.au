...
class MDN_BarcodeLabel_AdminController extends Mage_Adminhtml_Controller_Action {
     ...
     * Return label preview
     */
    public function LabelPreviewOrderAction() {
        //get datas
        $orderId = $this->getRequest()->getParam('order_id');
        $productId = $this->getRequest()->getParam('product_id');
        $vendorId = $this->getRequest()->getParam('vendor_id');
       // $img = Mage::helper('BarcodeLabel/Label')->getImageOrder($orderId,$productId);
        $img = Mage::helper('BarcodeLabel/Label')->getImageOrder($orderId,$productId,$vendorId);//04_03_2014
     ...
     * Print labels for 1 product
     */
    public function PrintProductLabelsOrderAction() {

        //Get data
        $productId = $this->getRequest()->getParam('product_id');
        $ordertId = $this->getRequest()->getParam('order_id');
        $count = $this->getRequest()->getParam('count');
        $param = array($productId.'-'.$ordertId => $count);

        //create PDF
        $pdfModel = Mage::getModel('BarcodeLabel/Pdf_Labels');
        $pdf = $pdfModel->getPdfOrder($param);
        $this->_prepareDownloadResponse(mage::helper('BarcodeLabel')->__('Barcode labels') . '.pdf', $pdf->render(), 'application/pdf');
    }
