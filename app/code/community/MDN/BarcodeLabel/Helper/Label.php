...
class MDN_BarcodeLabel_Helper_Label extends Mage_Core_Helper_Abstract {
     ...
     * @param <type> $product 
     */
    protected function addincreamentid(&$im, $order) {

        if (Mage::getStoreConfig('barcodelabel/name/print') != 1)
            return false;

        $name = $order->getIncreamentId();
        $fontSize = Mage::getStoreConfig('barcodelabel/name/font_size') * $this->_coef;
        $position = $this->getPositions(Mage::getStoreConfig('barcodelabel/name/position'), true);
        $black = imagecolorallocate($im, 0, 0, 0);
        $font = $this->getFont();
...
protected function addBarcodeorder(&$image, $order,$product,$vendorId='') {
        ...
        //get barcode image
        //$barcodeAttributeName = Mage::helper('BarcodeLabel')->getBarcodeAttribute();
        //$barcode = $order->getIncrementId().'-'.$product->getSku();
        $barcode = $order->getIncrementId().'-'.$product->getSku().'-'.$vendorId;//04_03_2014
        $barcodeImage = Mage::helper('BarcodeLabel/BarcodePicture')->getImage($barcode);
     ...
     public function getImageOrder($orderId,$productId,$vendorId='') {
        ...
        //add barcode
        $this->addBarcodeorder($im, $order,$product,$vendorId);

        //add product name
       // $this->addincreamentid($im, $order);
