...
class MDN_Organizer_TaskController extends Mage_Adminhtml_Controller_Action {
        ...
        try {
            ...
            //    $Task->setot_created_at(date('Y-m-d H:i'));
                ...
                if($this->getRequest()->getPost('ot_id') != '' and $this->getRequest()->getPost('ot_entity_type') != 'product')
                {
                    ...
                    if($Task1->getot_deadline() != $this->getRequest()->getPost('ot_deadline'))
                    {
                        ...
                        /*
                        $sqlChain2 = $collection->getSelect()
                                                ->joinLeft(array('chain'=>$temptableChain, 'task'=>$temptableTask), 'chain.task_id = task.ot_id')
                                                ->where("chain.order_quote_id = '".$this->getRequest()->getPost('ot_entity_id')."'
                                                        AND chain.task_type = 'Chain' AND chain.product_id = '".$this->getRequest()->getPost('ot_item')."' AND task.ot_entity_type = '".$this->getRequest()->getPost('ot_entity_type')."'")
                                                ->order('chain.entity_id ASC');*/
                        $chkChain2 = $connectionRead->fetchAll($sqlChain2);
            ...
            if($this->getRequest()->getPost('ot_todo') == 1 and $this->getRequest()->getPost('ot_id') == '')
            {
               ...
               // $chkSaleOrder = Mage::getSingleton('core/resource')->getConnection('core_read')->query($sqlSaleOrder);
                ...
                $data = array();
                $data['ot_id']= $Task->getot_id();
                $data['user_id']=$userId;
                if($list_id!=''){
                     $data['list_id']=$list_id;
                }
                $connectionWrite->insert($temptableTodo, $data);
            ...
            if($this->getRequest()->getPost('ot_entity_type') != 'product' and count($chkChain1) == 0)
            {
                ...
                */
                ...
                $data['order_quote_id']=$this->getRequest()->getPost('ot_entity_id');
                if($this->getRequest()->getPost('ot_item') != ''){
                    $data['product_id']= $this->getRequest()->getPost('ot_item');
                }
                $data['task_type']= $this->getRequest()->getPost('ot_type');
            ...
            else if($this->getRequest()->getPost('ot_entity_type') != 'product')
            {
               ...
               // $chkChain = Mage::getSingleton('core/resource')->getConnection('core_write')->query($sqlChain);
                ...
                $data['task_type'] = $this->getRequest()->getPost('ot_type');
                $data['task_status'] = 'complete';
                $where = $connectionWrite->quoteInto('task_id =?', $Task->getot_id());
                $connectionWrite->update($temptableChain, $data, $where);
                $connectionWrite->commit();
            }
            ...
            Mage::dispatchEvent('model_save_after', array('object'=>Mage::getModel('Organizer/Task')));
                mage::helper('AdminLogger')->updatelog($Task->getot_id(),'Add or Edit Task');
            /********************* End by dev *****************************/
    ...
    public function DeleteAction() {
        ...
        Mage::dispatchEvent('model_save_after', array('object'=>Mage::getModel('Organizer/Task')));
        mage::helper('AdminLogger')->updatelog($otId,'Delete Task');
