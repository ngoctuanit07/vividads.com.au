...
//        }
    ...
    public function SaveAction() {
        ...
        try {
            ...
            /******************** Start for show the ticket comment in order page 21_04_2014 ************************/
                $connectionRead = Mage::getSingleton('core/resource')->getConnection('core_read');
                $connectionWrite = Mage::getSingleton('core/resource')->getConnection('core_write');
                
                $id = explode('_',$ticket->getct_object_id());
                $order_id = $id[1];
                ...
                $data['comment']=$newMessageData['ctm_content'];
                $data['created_at']= NOW();
                $data['entity_name']= 'order';
                $connectionWrite->insert($tableHistory, $data);
                $connectionWrite->commit();
                $ticket->notify(MDN_CrmTicket_Model_Message::AUTHOR_ADMIN);
            /******************** Start for show the ticket comment in order page 21_04_2014 ************************/
    ...
    public function AffectToCustomerAction() {
       ...
       $request = $this->getRequest();
        ...
        if($orderId && $ticketId && $customerId){
			...
			if($ticket){
              if($type=='quote'){
			  $ticket->setct_object_id('quote'.MDN_CrmTicket_Model_Customer_Object_Abstract::ID_SEPARATOR.$orderId);
			  }else{
			  ...
			  if(count($c_order->getData()) >0) { 
			    $orderId = $c_order->getIncrement_id();			  
              	$message = $helper->__('Ticket #%s assigned to Order # %s', $ticketId, $orderId);
			  }else{
				$c_order = Mage::getModel('Quotation/Quotation')->load($orderId);
				$orderId = $c_order->getIncrement_id();
				$message = $helper->__('Ticket #%s assigned to Quote # %s', $ticketId, $orderId); 
				  }
            }else{
