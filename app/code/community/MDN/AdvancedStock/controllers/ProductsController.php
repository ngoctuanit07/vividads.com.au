...
class MDN_AdvancedStock_ProductsController extends Mage_Adminhtml_Controller_Action {
        ...
        try {
			...
			foreach ($stocks as $stock) {
                ...
                foreach ($stockData as $key => $value) {
					...
					//Zend_debug::dump($stockData);exit;
                }
    ...
    public function SaveAction() {
        ...
        try {
            ...
            $stockData = $this->getRequest()->getPost('stocks');
            $stocks = mage::helper('AdvancedStock/Product_Base')->getStocks($productId);
			//stocks movement and save data 
			
			$currentStocks = Mage::getModel('AdvancedStock/StockMovement')
								->getCollection()
			...
			if(count($currentStocks)>0){
				foreach($currentStocks as $currentStock){
					$cQty[] = $currentStock->getSm_qty(); 
					}
			}
			$totalQty = array_sum($cQty);
            ...
            foreach ($stocks as $stock) {
				...
				$stock->save();
				///update stock  
			$update = Mage::getModel('cataloginventory/stock_item')->updateStockItems($productId, $totalQty);
