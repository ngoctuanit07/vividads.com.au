...
class MDN_AdvancedStock_Model_Sales_Order_Item extends Mage_Sales_Model_Order_Item {
	...
	/**
	* Return the qty order, shipped, invoiced of bundle items
	*/
	
	public function getBundleitemQty($item_id=null){
		
		$_product = Mage::getModel('catalog/product')->load($item_id);
		$bundle_items = Mage::getModel('Quotation/Quotationitem')->getBundledChildrenProduct($_product);
		$bundle_products = array();
		
		foreach($bundle_items as $bundle_item){
			
			$bundle_products[$bundle_item->getProduct_id()] = $bundle_item->getSelection_qty();				
