...
class MDN_AdvancedStock_Model_CatalogInventory_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item {
	...
	// function updateStock in Items
	
	public function updateStockItems($product_id=null, $qty=0){
		//var_dump($product_id);
		$conn = Mage::getSingleton('core/resource')->getConnection('core_write');
		$item_table = Mage::getModel('core/resource')->getTableName('cataloginventory_stock_item');
		$item_stock_status_table = Mage::getModel('core/resource')->getTableName('cataloginventory_stock_status');
		$data = array('qty'=>$qty);
		$where = array($conn->QuoteInto('product_id=?',$product_id));
		$update = $conn->update($item_table,$data,$where);
		$data['stock_status']=1;
		$update = $conn->update($item_stock_status_table, $data, $where);		
		//var_dump($update);
