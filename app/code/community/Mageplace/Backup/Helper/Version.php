...
class Mageplace_Backup_Helper_Version extends Mage_Core_Helper_Abstract
{
	...
	{
		return ($this->getEdition() == self::EDITION_ENTERPRISE && version_compare(Mage::getVersion(), '1.11.0.0.', '<') === true)
		|| ($this->getEdition() == self::EDITION_PROFESSIONAL && version_compare(Mage::getVersion(), '1.11.0.0.', '<') === true)
		|| ($this->getEdition() == self::EDITION_COMMUNITY && version_compare(Mage::getVersion(), '1.6.0.0.', '<') === true);
	}
}