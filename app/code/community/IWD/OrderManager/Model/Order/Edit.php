...
class IWD_OrderManager_Model_Order_Edit extends Mage_Sales_Model_Order_Item
{
    ...
    public function calculateGrandTotal($order)
    {
        ...
        $grand_total = $order->getSubtotal() + $order->getTaxAmount() + $order->getShippingInclTax() - abs($order->getDiscountAmount());
        $base_grand_total = $order->getBaseSubtotal() + $order->getBaseTaxAmount() + $order->getBaseShippingInclTax() - abs($order->getBaseDiscountAmount());
